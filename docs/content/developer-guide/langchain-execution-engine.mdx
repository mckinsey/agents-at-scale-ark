...
To enable stateful conversations with memory persistence, you must install a Memory resource. The [PostgreSQL Memory Service](./services/postgres-memory) provides a production-ready memory implementation that can be used with this execution engine.
...
- [PostgreSQL Memory](./services/postgres-memory) - Persistent memory storage
- [A2A Gateway](./a2a-server) - Agent-to-agent communication
