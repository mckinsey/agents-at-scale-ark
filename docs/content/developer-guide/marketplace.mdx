  ...
  "items": [
    {
      ...
      "description": "Adds X capability to Ark agents",
      "version": "0.1.0",
      "category": "mcp-servers",
      "tags": ["mcp", "internal"],
      "license": "Apache-2.0",
      "homepage": "https://github.com/my-org/my-mcp",
      "documentation": "https://my-org.github.io/my-mcp/",
      "ark": {
        "chartPath": "oci://ghcr.io/my-org/charts/my-mcp-server",
        "namespace": "default",
        "helmReleaseName": "my-mcp-server",
...
## What needs to be publicly reachable?

The fields in the manifest are fetched from three different places. Only the manifest itself sits on a critical path.

| Field | Fetched by | Hosting requirement |
| --- | --- | --- |
| The manifest URL | **Dashboard pod**, server-side, no `Authorization` header | Must be reachable anonymously from inside the cluster. Public HTTPS (e.g. `raw.githubusercontent.com`) is the easy default. A private GitHub repo won't work unless you front it with an anonymously-readable URL. |
...
| `icon` | **Not fetched.** Rendered as plain text in the item detail view. | Designed for emoji strings like `"🔥"`. A URL here will render as literal text, not as an image. Image hosting is not required today. |
| `screenshots` | **Not rendered.** Accepted by the schema but no UI consumes the field. | Image hosting is not required today. |
| `homepage`, `documentation`, `repository`, `support.url` | **The user's browser** when they click the link | Should be reachable from your users' networks, like any external link. |

Practical implication: for a fully-internal marketplace, you need exactly one publicly-fetchable URL — the manifest. Charts can live in your private OCI registry; icons can stay as emoji.
...
- **Sources persist in `localStorage`, per browser.** Sources added under **Settings → Manage marketplace** are saved with Jotai's `atomWithStorage` keyed to `marketplace-sources`. They live only in the browser that added them — not synced to the cluster, not shared across users, and lost if you clear site data.
...
- **No authentication for source URLs.** The dashboard fetches manifests with no `Authorization` header (see [What needs to be publicly reachable?](#what-needs-to-be-publicly-reachable)). Private GitHub repos and authenticated endpoints can't host a manifest unless you front them with an anonymously-readable URL.
...
- **No per-source status indicator.** If a source URL becomes unreachable, its items silently drop out of the grid. The error surfaces in the browser console, not the UI.
