...
- **No authentication for source URLs.** The validate endpoint fetches the manifest with no `Authorization` header. Private GitHub repos and authenticated registries can't host the manifest unless they expose an HTTPS endpoint that's reachable anonymously (e.g. a signed pre-resolved URL or a publicly readable raw file).
