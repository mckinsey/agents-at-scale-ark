...
- **Custom URL** — any HTTPS manifest URL, public or authenticated (GitHub raw, GitHub Enterprise Server, artifact stores, etc.). See [Custom URL](#custom-url).
- **Azure DevOps** — a preset for Azure DevOps Repos that builds the fetch URL from repo details instead of a free-text URL. See [Azure DevOps](#azure-devops).

#### Custom URL

This example adds a **public** source — leave **Authentication** on **None**; the authenticated case is covered right after, in [Add an authenticated source](#add-an-authenticated-source).
...
| Field | Notes |
| --- | --- |
| **Organization** | The Azure DevOps organization name, from `https://dev.azure.com/<org>/...`. |
| **Project** | The project name within the organization. |
| **Repository** | The Git repository name within the project. |
| **Branch** | Defaults to `main`. Change it to target a different branch. |
| **Path** | Defaults to `/marketplace.json`. Change it if the manifest lives elsewhere in the repo. |
...
![Private Azure DevOps repository](/marketplace/azure-ado-hierarchy.png)
...
![Add new marketplace form in Azure DevOps mode with generated URL preview](/marketplace/azure-devops-source.png)
...
| Field | What it does |
| --- | --- |
| **Authentication** | Public source — select **None**. GitHub raw, GitHub Enterprise, or an artifact store (Artifactory, Nexus, Harbor) — select **Bearer / token**. Azure DevOps Repos — select **HTTP Basic (Azure DevOps)**. |
