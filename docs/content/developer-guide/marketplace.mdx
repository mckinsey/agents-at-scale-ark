...
In **Settings → Manage marketplace → Add new marketplace**, enter the manifest URL, pick **Bearer / token** or **HTTP Basic**, and paste the token. The token is
sent once on save and validated against the manifest before the source is stored; it is never displayed again. For Azure DevOps, use HTTP Basic and paste the PAT (the empty username is handled for you).
