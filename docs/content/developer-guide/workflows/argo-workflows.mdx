...
Open the Argo dashboard with at [`http://localhost:2746`](http://localhost:2746) and choose "Workflows > New Workflow > From Template > Query Fanout".
...
argo submit --from workflowtemplate/query-fanout-template \
  ...
  -p evaluator-agent="sample-agent"
