...
This installs Argo in single-namespace mode within the Ark tenant namespace (typically `default` in development). Production installations typically use a cross-tenant Argo operator with execution distributed across Ark tenant namespaces.
...
1. **The `ark` CLI** — run the pre-built [`ark-tools`](/reference/utility-images#ark-tools) image (which bundles `ark`, `fark`, `kubectl`, `helm`, and `argo`) and call `ark query <target> "<message>"`. Simplest for one-shot calls.
...
To thread several steps into one conversation (shared memory) and group them under a session for telemetry, set `conversationId` and `sessionId`. The `ark` CLI exposes `--conversation-id` / `--session-id`:
