...
fark agent weather "How about tomorrow?" --session-id weather-session --conversation-id weather-conv
```
...
| `--force` | Bypass the preflight refusal when `status.authorization.state` is already `Authorized`. Maps to `force: true` on `auth/start`. |
| `--force-registration` | Trigger a fresh RFC 7591 dynamic client registration even if cached credentials exist. Orthogonal to `--force` — does not bypass the preflight. |
...
For operator-side configuration (the callback URL, RBAC, port-forward recipe), see [MCP OAuth Callback](/operations-guide/mcp-oauth-callback).
