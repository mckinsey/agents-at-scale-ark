...
Add new functionality to the appropriate overlay files:

```bash
# Example structure for new functionality
lib/ark-sdk/gen_sdk/overlay/python/ark_sdk/
├── new_client.py        # Add client class
├── models/
│   └── models.py        # Add data models
...
- **Generated clients** → Modify `python_sdk.py` to inject new client

### 2. Build Local Wheel File

Generate the ark-sdk with changes:

```bash
# Clean previous builds
rm -rf out/ark-sdk

# Build ark-sdk with changes
make ark-sdk-build
...
Copy the wheel to the consuming service for testing and use the new ark-sdk functionality

```bash
# Copy wheel to the service (e.g., ark-api)
...
from ark_sdk.new_client import NewClient
from ark_sdk.models.models import Model, Request

# Initialize the client
client = NewClient(namespace="default")
...
Deploy consuming service with the new ark-sdk dependency:

```bash
# Build and deploy service
make <service_name>-build
devspace deploy
