...
The ARK SDK supports overlay files for adding custom functionality beyond generated types:

1. **Add overlay files**: Place custom Python modules in `lib/ark-sdk/gen_sdk/overlay/python/ark_sdk/`
2. **Rebuild SDK**: Run `make ark-sdk-build` to include overlay files in the wheel
3. **Automatic rebuilds**: The build system tracks overlay files and rebuilds when they change

Example overlay files include:
- `k8s.py` - Kubernetes client utilities
- `client.py` - ARK client wrapper
- `streaming_config.py` - Streaming configuration utilities

Overlay files are copied into the generated SDK during build and packaged in the wheel.
