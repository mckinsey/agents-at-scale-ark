...
This guide shows you how to create A2A servers that host custom agents. Exposing agents via A2A servers allows you to develop agents using your preferred tools and frameworks, such as LangChain, CrewAI, or custom agent code and expose them to Ark using the A2A protocol.
...
The fastest way to get started is with the [Simple Agent sample](https://github.com/mckinsey/agents-at-scale-ark/tree/main/samples/a2a/simple-agent). This sample has the following basic structure:
...
The sample's response logic is a deliberately simple, dependency-free keyword matcher — it is not an LLM agent. The point is the A2A plumbing, not the intelligence. In `src/simple_a2a_server/main.py`, `SimpleAgentExecutor._process_message` is the seam where a real implementation would call your LLM client or agent framework (LangChain, CrewAI, custom code); everything around it — the agent card, the A2A server, and Ark discovery — stays the same.
...
Rather than running the sample agent locally, you can run it in the cluster. For **development**, [`devspace`](http://devspace.sh) runs your code in a container with live reload so edits are reflected without a rebuild:

```bash
# Start the a2a server with live reload (development).
cd samples/a2a/simple-agent
devspace dev
```

For a **staging or production** deployment, build a versioned image and apply the manifests instead — see [Deploying to staging or production](#deploying-to-staging-or-production) below.

Running with devspace applies the resources in the `manifests.yaml` file. These are:
...
# simple-agent   True    False         http://simple-agent.default.svc.cluster.local:80   6m13s
```
...
The agent has no `modelRef` (A2A agents bring their own model) and uses the built-in `a2a` execution engine. It may report `AVAILABLE   False` with the message `ExecutionEngine 'a2a' not found` — this is an availability-probe artifact; the agent is still queryable, as the query above shows.

### Viewing in the dashboard

The dashboard surfaces both the A2A server and the agent Ark created from it. Open it with `ark dashboard`.

The **A2A Servers** page lists the registered server with its status and resolved address:

![Ark dashboard A2A Servers page showing the simple-agent server as Available](./images/a2a-dashboard-servers.png)

Under **Agent Builder → Agents**, the discovered agent appears tagged as an **A2A Agent**:

![Ark dashboard Agents list showing simple-agent tagged as an A2A Agent](./images/a2a-dashboard-agents.png)
...
`devspace dev` is a development convenience — it swaps in a live-reload container and only lasts as long as the command runs. For staging or production you deploy a **fixed, versioned image** through your normal delivery mechanism. The sample's `manifests.yaml` is plain Kubernetes (a `Deployment` and `Service`) plus a single Ark `A2AServer`, so it drops into `kubectl`, Kustomize, Helm, or a GitOps tool (Argo CD / Flux) unchanged.

**1. Build and push a versioned image** to a registry your cluster can pull from:

```bash
cd samples/a2a/simple-agent
docker build -t <registry>/simple-agent:v1.0.0 .
docker push <registry>/simple-agent:v1.0.0
```

**2. Point the manifests at that image.** In `manifests.yaml`, set the `Deployment` image to the pushed tag and avoid the floating `:latest` tag so rollouts are deterministic:
...
spec:
  template:
    spec:
      containers:
      - name: simple-agent
        image: <registry>/simple-agent:v1.0.0
        imagePullPolicy: IfNotPresent
```

**3. Apply the manifests** to the target cluster (commit them to your GitOps repo instead if that's your workflow):

```bash
kubectl apply -f manifests.yaml
```

**4. Verify** exactly as in development — the `A2AServer` becomes `Ready`, Ark creates the `Agent`, and it answers queries:
...
Discovery takes a few seconds. Wait for the `A2AServer` to report `READY True` (and the `Agent` to appear) before querying — until then the agent does not yet exist and queries are rejected.

For production, harden the `Deployment` the way you would any workload: pin the image tag, set `replicas`, keep resource requests/limits (the sample already sets them), and add liveness/readiness probes against the `/health` endpoint. If the A2A server needs its own credentials (API keys for the framework behind it), mount them from a `Secret` rather than baking them into the image — this is separate from the per-query header overrides Ark forwards to the agent.
