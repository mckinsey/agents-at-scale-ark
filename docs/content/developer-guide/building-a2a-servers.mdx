...
1. **Query.spec.timeout** - Per-query timeout specified when creating a Query resource
2. **A2AServer.spec.timeout** - Default timeout for all agents served by this A2A server
3. **A2A_DEFAULT_TIMEOUT** environment variable - Deployment-wide default (ark-api-a2a gateway)
4. **Hardcoded default** - 5 minutes (300 seconds)
...
- **Set reasonable defaults**: Configure A2AServer timeout based on typical agent response times
- **Use Query timeout for exceptions**: Override timeout for queries you know will take longer
- **Account for overhead**: Set A2AServer timeout slightly higher than expected agent processing time
