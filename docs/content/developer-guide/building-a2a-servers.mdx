...
- **`Task`**: Ark reads the final answer from, in order of precedence:
  1. Agent messages in `history`
  2. The terminal `status.message` (or a final `TaskStatusUpdateEvent` when streaming)
  3. Text `artifacts` (`artifacts[].parts[].text`, or `TaskArtifactUpdateEvent` when streaming)

A completed `Task` that returns its output as a text artifact is the canonical A2A pattern (the spec states completed tasks should use artifacts to return generated output), and Ark picks it up. History and `status.message` are checked first, so agents that report results either way work unchanged.
