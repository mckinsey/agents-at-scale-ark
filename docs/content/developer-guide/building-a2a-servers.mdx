...
Rather than running the sample agent locally, you can run it in the cluster with [`devspace`](http://devspace.sh), which runs your code in a container with live reload so edits are reflected without a rebuild:
...
Your A2A server can reply with either a stateless `Message` or a stateful `Task`. Ark extracts the response text from both:

- **`Message`**: Ark reads the text parts of the message.
- **`Task`**: Ark reads the final answer from, in order of precedence:
  1. Text `artifacts` (`artifacts[].parts[].text`, or `TaskArtifactUpdateEvent` when streaming)
  2. The terminal `status.message` (or a final `TaskStatusUpdateEvent` when streaming)
  3. Agent messages in `history`

A completed `Task` that returns its output as a text artifact is the canonical A2A pattern (the spec states completed tasks should use artifacts to return generated output), so Ark checks artifacts first. The terminal `status.message` and `history` are used as fallbacks, so agents that report results either way work unchanged.

Only text is extracted as the response. File and data parts are recorded on the `A2ATask` resource status for observability but are not returned as the query answer.
...
1. **Query.spec.timeout** (default: 30 minutes) sets the overall time limit
2. **A2AServer.spec.timeout** (optional) can reduce the timeout for specific A2A servers

The A2A execution uses Go's context deadline, so remaining time is automatically tracked as the query progresses. Any `spec.timeout` takes effect — the context deadline is authoritative, including values above the 30 minute default. The HTTP client applies a 30 minute backstop only to callers that set no deadline of their own; a query always carries its own budget, so the backstop never caps it.
