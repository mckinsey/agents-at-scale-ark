...
The agent has no `modelRef` (A2A agents bring their own model) and uses the built-in `a2a` execution engine. It becomes `AVAILABLE   True` once its `A2AServer` is `Ready`.
