...
When Ark sends a message to an A2A agent, it may include extension metadata keyed by URI (e.g. conversation history, delegated permissions). Which extensions are included depends on the agent's `ark.mckinsey.com/a2a-supported-extensions` annotation:

- **No annotation**: History is included by default. Permissions are excluded unless explicitly declared.
- **Annotation present**: Only URIs listed in the annotation receive metadata.

This annotation is populated automatically from the AgentCard's `capabilities.extensions` during A2AServer discovery. See [Agent - Supported Extensions](/reference/resources/agent#a2a-supported-extensions-annotation) for the annotation format and [A2A Native Execution - Extension Negotiation](/reference/a2a-native-execution#extension-negotiation-and-gating) for the full gating rules.
