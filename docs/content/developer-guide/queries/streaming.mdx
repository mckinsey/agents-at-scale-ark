...
Streaming is implemented using Server-Sent Events (SSE) with chunks in OpenAI-compatible format. Streaming is enabled cluster-side by configuring an event streaming service (see [Event Streaming Service](#event-streaming-service)); no per-query flag is required. Consume SSE from the broker proxy endpoint using the query name. See the [Ark APIs](/reference/ark-apis) reference for details.

## Enabling Streaming

Streaming is a cluster-side capability, not a per-query setting. When an event streaming service is configured, every query produces chunks that can be consumed from the broker proxy. Create a query as usual and stream its chunks by query name:
...
When this ConfigMap is present and `enabled` is set to `"true"`, the Query Controller will use the specified service for event streaming. If the ConfigMap is not present or `enabled` is `"false"`, streaming is disabled and queries return only their final response.
