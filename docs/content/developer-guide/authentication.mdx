...
OIDC_SCOPES="openid email profile"                   # Optional: Space-separated OAuth scopes (defaults to "openid email profile")
...
By default the dashboard requests `openid email profile`:
- `openid` - Core OIDC functionality
- `profile` - User profile information
- `email` - User email address

Override the requested scopes with the `OIDC_SCOPES` environment variable (space-separated). This is useful when a provider requires additional scopes. For example, some providers (such as Dex) only issue refresh tokens when `offline_access` is requested, which is required to keep sessions alive:

```bash
OIDC_SCOPES="openid email profile offline_access"
```

If `OIDC_SCOPES` is unset or empty, it falls back to the default `openid email profile`.
...
- If the provider only issues refresh tokens for the `offline_access` scope (e.g. Dex), add it via `OIDC_SCOPES="openid email profile offline_access"`
