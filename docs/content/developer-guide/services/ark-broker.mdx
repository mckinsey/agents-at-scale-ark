...
Heap pressure scales with the in-memory stores, so `limits.maxMessages`, `limits.maxChunks`, `limits.maxSpans` and `limits.maxEvents` are the other lever. Moving messages to Postgres or chunks to Redis takes those stores off the heap; traces and sessions stay in memory regardless.
...
A gauge is registered only for stores actually held in the heap — with `MESSAGE_BACKEND=postgres`, `broker_messages_count` is absent rather than reported as zero.
