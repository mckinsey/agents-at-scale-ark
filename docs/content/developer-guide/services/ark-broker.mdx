...
- In-memory message storage by default, optionally persisted to disk / persistent volume, or backed by Postgres for the messages and operation events stores
...
By default the broker keeps all stores in memory. The **messages** store can opt in to Postgres so messages survive pod restarts. This section covers the messages store; operation events have their own Postgres opt-in (see [Event backend (Postgres)](#event-backend-postgres)) and completion chunks can opt in to Redis (see [Chunk backend (Redis)](#chunk-backend-redis)), while traces and sessions remain in memory (or file-based, as above).
