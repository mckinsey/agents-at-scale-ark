...
| `SHUTDOWN_DRAIN_TIMEOUT_MS` | On SIGTERM, how long to let in-flight requests (including open `/stream` responses) finish before connections are cut and the brokers are closed | `10000` | — |
| `MESSAGE_MAX_BYTES` | Byte budget for the in-memory message store | `104857600` (100 MiB) | `limits.messageMaxBytes` |
| `EVENT_MAX_BYTES` | Byte budget for the in-memory event store | `104857600` (100 MiB) | `limits.eventMaxBytes` |
| `CHUNK_MAX_BYTES` | Byte budget for the in-memory chunk store | `33554432` (32 MiB) | `limits.chunkMaxBytes` |
| `TRACE_MAX_BYTES` | Byte budget for the in-memory trace store | `33554432` (32 MiB) | `limits.traceMaxBytes` |
| `CHUNK_TTL_SECONDS` | Retention window for streaming chunks, in seconds | `3600` | `limits.chunkTtlSeconds` |
...
File persistence stores each stream as a single append-only file on a `ReadWriteOnce` volume, written by exactly one broker. Run **one broker replica per store**: two brokers writing the same volume duplicate or silently lose events. With persistence enabled the chart sets the Deployment strategy to `Recreate` so a rollout never briefly runs two brokers against the same volume. This is a limit **per store, not per cluster** — a broker per namespace is safe, since each has its own volume, file, and sequence space. For high availability with shared state, use the Postgres (messages, events, sessions) and Redis (chunks) backends instead of file persistence.
...
Heap pressure scales with the in-memory stores, so the per-store byte budgets `limits.messageMaxBytes`, `limits.eventMaxBytes`, `limits.chunkMaxBytes` and `limits.traceMaxBytes` are the other lever. Moving messages or sessions to Postgres, or chunks to Redis, takes those stores off the heap; traces stay in memory regardless. On the default in-memory backend the sessions index has no size limit — it grows with the queries it has seen, and `DELETE /sessions/queries/{queryId}` drops one query at a time, so nothing clears it wholesale. Restarting the pod does, unless file persistence is enabled (`persistence.enabled`), in which case the index is reloaded from `SESSIONS_FILE_PATH` on boot and the restart costs nothing. It is the one heap consumer with no cap; move sessions to Postgres if that growth matters.
...
Messages carry a TTL-based expiry: each message is written with an `expires_at` snapshot and filtered out once it elapses. The expiry aligns with the originating query's TTL (`spec.ttl`, defaulting to `ArkConfig.spec.queryTTL` with a 720h fallback), and falls back to `MESSAGE_VISIBILITY_TTL_SECONDS` when no per-message TTL is supplied. Expired rows are also physically deleted: a background reaper runs every `ROW_REAP_INTERVAL_SECONDS` (hourly by default) and deletes rows past `expires_at` in batches of `ROW_REAP_BATCH_SIZE`, so the tables don't grow unbounded. The reaper runs independently in every broker replica: `FOR UPDATE SKIP LOCKED` makes concurrent reapers skip each other's rows instead of blocking, so the only cost of N replicas is N times the scan per interval.
...
Session expiry is a **sliding** window rather than the write-time snapshot used for messages and events: every event or message on a session pushes `expires_at` out by `SESSIONS_VISIBILITY_TTL_SECONDS`, so a session stays visible for as long as it is in use, and the value does not track the originating query's `spec.ttl`. The minimum enforced at startup exists because a session is the index into its own messages and events — hiding it earlier than they expire would strand data that is still retained. Expiry hides a session from reads, and a later event or message on a still-present expired session makes it visible again. The expired-row reaper covers the `sessions` table too (see the [message backend](#message-backend-postgres)), so a session that stays expired is eventually deleted, taking its `session_queries` rows with it via `ON DELETE CASCADE` — after that there is no row left to revive.

Deleting a `Query` removes it from the sessions read model. The controller's finalizer calls `DELETE /sessions/queries/{queryId}` alongside the message and event cleanup paths (see [Query → Deletion and cleanup](/reference/resources/query#deletion-and-cleanup)): the `session_queries` row goes, the session header's aggregates are recomputed, and a session left with no queries is deleted with it.
...
| `REDIS_STREAM_TTL_SECONDS` | `3600` | Deprecated alias for `CHUNK_TTL_SECONDS`; honored if set, but prefer `CHUNK_TTL_SECONDS` |
...
| DELETE | `/sessions/queries/{queryId}` | Remove one query from every session holding it |

Clearing memory does not touch this index. `DELETE /conversations` — the dashboard's **Reset memory** — empties the message store and leaves every session, query and conversation summary standing, because a session's aggregates are folded from its own query rows rather than from messages. Removing a query from the index is what `DELETE /sessions/queries/{queryId}` above does, and the controller's finalizer calls it when a `Query` is deleted.
