...
Heap pressure scales with the in-memory stores, so `limits.maxMessages`, `limits.maxChunks`, `limits.maxSpans` and `limits.maxEvents` are the other lever. Moving messages to Postgres or chunks to Redis takes those stores off the heap; traces and sessions stay in memory regardless. The sessions index has no size limit — it grows with the queries it has seen until `DELETE /sessions` purges it.
...
A gauge is registered only for stores actually held in the heap — with `MESSAGE_BACKEND=postgres`, `broker_messages_count` is absent rather than reported as zero. Traces and sessions have no backend to move to, so their gauges are always present.
