...
The A2A Gateway exposes ARK agents via the [A2A Protocol](https://github.com/a2aproject/A2A) for standardized agent-to-agent communication. All ARK agents in Kubernetes are automatically discovered and exposed. Streaming is supported by default when a broker is available.

**Endpoints:**
- `GET /a2a/agents` - List all available agents
- `GET /a2a/agent/{agent-name}/.well-known/agent-card.json` - Get agent card
- `POST /a2a/agent/{agent-name}/` - A2A JSON-RPC routes (`message/send`, `message/stream`)
...
# Get agent card
curl http://localhost:8000/a2a/agent/<agent-name>/.well-known/agent-card.json
```

<Callout type="warning">
**Experimental.** An opt-in internal A2A transport mode replaces the default OpenAI wire format with native A2A messages. Enable it by setting `ark.mckinsey.com/a2a-experimental-enabled: "true"` on agent, team, or query resources. This feature is subject to change or withdrawal. See the [RFC: Experimental A2A transport](/reference/a2a-experimental-rfc) for full details.
</Callout>
