...
| `ARK_A2A_LISTEN_PORT` | Port for A2A agent card URLs | Uses `PORT` (8000) |
| `ARK_A2A_LISTEN_HOST` | Host for A2A agent card URLs | `localhost` |
| `ARK_A2A_LISTEN_PROTOCOL` | Protocol scheme for A2A URLs | `http` |
| `ARK_A2A_LISTEN_PATH` | Optional path prefix for A2A URLs | Empty (root path) |
