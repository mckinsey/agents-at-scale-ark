...
**Experimental.** An opt-in internal A2A transport mode replaces the default OpenAI wire format with native A2A messages. Enable it by setting `ark.mckinsey.com/a2a-experimental-enabled: "true"` on agent, team, or query resources. This feature is subject to change or withdrawal. See the [A2A Native Execution reference](/reference/a2a-native-execution) for full details.
</Callout>
