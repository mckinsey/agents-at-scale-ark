...
# Show structured event data from annotations
kubectl get events --field-selector involvedObject.kind=Query -o json | jq -r '.items[] | select(.metadata.annotations."ark.mckinsey.com/event-data") | {reason: .reason, data: (.metadata.annotations."ark.mckinsey.com/event-data" | fromjson)}'
...
As an example, query execution emits these events following the `{Operation}{State}` pattern:

- `Normal/QueryExecutionStart`: Query begins execution
- `Normal/AgentExecutionStart`: Agent begins execution
- `Normal/ToolCallStart`: Tool invocation begins
- `Normal/ToolCallComplete`: Tool invocation completes with duration and results
- `Normal/LLMCallComplete`: LLM call completes with token usage details
- `Normal/AgentExecutionComplete`: Agent execution completes with duration
- `Normal/QueryExecutionComplete`: Query execution completes

All events with structured data store it in the `ark.mckinsey.com/event-data` annotation as JSON.
...
The new eventing system uses specialized recorders from the `eventing` package. Each resource type has a dedicated recorder:
...
// In controller setup
r.Eventing = eventing.NewRecorders(eventEmitter)

// In reconciler - use operation recorders with Start/Complete/Fail pattern
ctx = r.Eventing.QueryRecorder().Start(ctx, "QueryExecution", "Executing query", nil)

// On success
r.Eventing.QueryRecorder().Complete(ctx, "QueryExecution", "Query execution completed", operationData)

// On failure
r.Eventing.QueryRecorder().Fail(ctx, "QueryExecution", "Query execution failed", err, nil)
```

The recorders automatically:
- Track timing and duration
- Add structured metadata to annotations (`ark.mckinsey.com/event-data`)
- Follow consistent naming patterns (`{Operation}Start`, `{Operation}Complete`, `{Operation}Error`)
- Include operation context (queryId, sessionId, namespace, etc.)
...
#### Unhandled error shapes

Errors the controller does not recognize are still surfaced, not replaced with a generic message. `extractStableError` in `ark/internal/controller/model_probe.go` matches the known shapes first (context deadline, `*openai.Error`, `*smithyhttp.ResponseError`, context cancellation), then falls back to `stabilizeProbeError`, which returns the error text with the unstable parts removed:

- Labelled identifiers and their values - `Trace ID`, `Correlation ID`, `Request ID`, `Activity ID`
- Bare UUIDs and hex tokens of 16 characters or more
- `Timestamp` values

The result is collapsed to a single line and capped at 256 characters. Two attempts that differ only in these tokens produce the same message, so the condition converges after one status write.

Extend `volatileTokenPatterns` when a provider introduces a new per-attempt identifier. Without it, a message that changes on every probe writes status, re-triggers reconciliation, and re-probes the provider.
