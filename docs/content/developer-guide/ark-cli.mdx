...
Evaluation commands follow a simplified syntax:

```bash
# Direct evaluation (with input/output)
ark evaluation <evaluator-name> --input "..." --output "..."

# Query-based evaluation
ark evaluation <evaluator-name> <query-name>
ark evaluation <evaluator-name> <query-name> --response-target agent:my-agent

# Optional flags for both types
ark evaluation <evaluator-name> ... --timeout 30s --watch-timeout 60s
```
...
Five main status codes are used:
...
| `4` | Evaluation Failed | Evaluation completed but test failed |
...
Query operations return exit code `2` when execution fails and `3` on timeout. Evaluation operations return exit code `4` when the evaluation completes successfully but the test fails (e.g., `passed: false`), allowing CI/CD pipelines to distinguish between operational failures and actual test failures. This behavior aligns with Argo Workflows' `--wait` flag pattern (see [argoproj/argo-workflows#1008](https://github.com/argoproj/argo-workflows/issues/1008)).
...
# Evaluation example
if ! ark evaluation my-evaluator my-query; then
   case $? in
      1) echo "CLI error: Invalid arguments" ;;
      2) echo "Operation error: Evaluation service failed" ;;
      3) echo "Timeout: Evaluation took too long" ;;
      4) echo "Test failed: Evaluation completed but test did not pass" ;;
   esac
fi
```
