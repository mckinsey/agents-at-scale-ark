...
Span attributes carry payload content — prompts, model messages, tool-call arguments, and query input/output. Before any span is exported, Ark redacts credentials at a **single choke point** (`convertAttribute` in the tracer), so every current and future span attribute is covered, upstream of all exporters (primary and per-tenant).

It uses the **same two-pass redaction** as the ark-api log filter (see the [Logging Contract](/operations-guide/logging-contract#credential-redaction)), driven by a shared pattern set and the same [test vectors](https://github.com/mckinsey/agents-at-scale-ark/blob/main/ark/internal/telemetry/redact/testdata/credential-redaction.json):

- **Key-anchored** — `access_token=…`, `client_secret=…`, `authorization: Bearer …` (including the same keys inside JSON tool-call arguments) → `[REDACTED]`.
- **Shape-based** — standalone tokens matching well-known formats (JWTs, OpenAI/Anthropic `sk-…`, GitHub `ghp_…`/`github_pat_…`, AWS `AKIA…`, Google `AIza…`, Slack `xox…`, Stripe `…_live_…`, PEM `PRIVATE KEY` blocks) → `[REDACTED]`, even with no adjacent key.

**Limitation:** this is pattern-based, not full content classification. A secret matching neither a known key nor a known token format — an opaque/custom credential, or PII — is not detected. Content-level PII/DLP remains a deployment responsibility: mask in your observability backend before long-term storage.
