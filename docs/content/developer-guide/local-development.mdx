...
The ark-controller supports three storage backends: etcd (default), SQLite, and PostgreSQL. Use the `STORAGE_BACKEND` environment variable to select:

```bash
# etcd (default) - uses Kubernetes CRDs
devspace dev

# SQLite - embedded apiserver with file storage
STORAGE_BACKEND=sqlite devspace dev

# PostgreSQL - deploys a postgres pod with embedded apiserver
STORAGE_BACKEND=postgresql devspace dev
```

When using `sqlite` or `postgresql`, the controller runs an embedded API server instead of using CRDs. Data is ephemeral in dev mode (SQLite uses `/tmp/ark.db`, PostgreSQL has no persistence enabled).
