...
Authoring inline tools is on in DevSpace already. To also run them, build the
runner images and install the activator:

```bash
INLINE_TOOLS_RUNTIME=true devspace dev
```

The activator refuses a cluster-wide RBAC binding, so this narrows the
controller to the `default` and `ark-system` namespaces — Ark resources
elsewhere are not reconciled. Try it with `kubectl apply -f
samples/tools/inline-tools.yaml`. Note that a local k3s or Docker Desktop
cluster usually runs a CNI that ignores NetworkPolicy, so the runner's network
isolation is not actually enforced there.
