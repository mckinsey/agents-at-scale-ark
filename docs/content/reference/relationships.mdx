...
- **Deprecated team strategies are migrated.** `round-robin` and standalone `graph` are accepted but rewritten by the mutating webhook (`round-robin` → `sequential` with loops; `graph` → `selector`). Author new teams with `sequential` or `selector`. See [Teams](/reference/resources/team).
