...
Storage defaults to in-memory, with pluggable persistent backends selected via environment variables: messages and operation events can be backed by PostgreSQL (`MESSAGE_BACKEND=postgres` / `EVENT_BACKEND=postgres`, sharing one `DATABASE_URL`) and chunks by Redis (`CHUNK_BACKEND=redis`, `REDIS_URL`). With a persistent backend configured, that data survives pod restarts.
