...
Storage defaults to in-memory, with pluggable persistent backends selected via environment variables: messages, operation events, and sessions can be backed by PostgreSQL (`MESSAGE_BACKEND=postgres` / `EVENT_BACKEND=postgres` / `SESSIONS_BACKEND=postgres`, sharing one `DATABASE_URL`) and chunks by Redis (`CHUNK_BACKEND=redis`, `REDIS_URL`). With a persistent backend configured, that data survives pod restarts.
...
- **Write vs read.** The controller and executors POST data in (chunks, traces, events, messages); the dashboard and `ark-api` GET it out. Deletes name what to remove — a conversation or a query. The one exception is `DELETE /conversations`, which clears the whole memory store.
...
`DELETE /conversations` clears the message store only — it backs the dashboard's **Reset memory** action. The [sessions](#sessions) read model survives it: a session keeps its queries, conversation summaries and participants, because those are folded from the session's own query rows rather than from messages. A query leaves a session when its `Query` is deleted, and a session left with no queries goes with it.
...
Held in memory by default, per replica. With `SESSIONS_BACKEND=postgres` sessions are stored in Postgres and `?watch=true` updates reach every replica's SSE clients — see [Sessions backend (Postgres)](/developer-guide/services/ark-broker#sessions-backend-postgres).

```
GET    /sessions
GET    /sessions/{sessionId}
POST   /sessions
DELETE /sessions/queries/{queryId}
```
...
| Domain | Endpoints |
|--------|-----------|
| Messages | `GET`/`POST` `/messages`, `GET /memory-status` |
...
| Chunks | `GET /stream`, `GET /stream/{query_name}`, `POST /stream/{query_id}`, `POST /stream/{query_id}/complete` |
| Traces | `POST /v1/traces` (OTLP), `GET /traces`, `GET /traces/{traceId}` |
| Events | `GET`/`POST` `/events`, `GET`/`DELETE` `/events/{queryId}` |
| Sessions | `GET`/`POST` `/sessions`, `GET /sessions/{sessionId}`, `DELETE /sessions/queries/{queryId}` |
