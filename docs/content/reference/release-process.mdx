...
- Python packages: 8 `pyproject.toml` files (ark-sdk, ark-api, ark-mcp, arkpy, postgres-eventsink, autogen-execution-engine, a2agw-python, overlay)
- Helm charts: 10 `Chart.yaml` files with both `version` and `appVersion` fields
...
Total: 58+ tracked files across the monorepo.
...
4. **RC Artifact Publishing** (Automatic on merge)
   ...
   - Attaches artifacts to the GitHub release:
     - **Python wheels** — 8 packages built from `lib/` and `services/` (job: `release-libs`)
     - **Helm charts** — 10 packaged charts as `.tgz` files (job: `release-charts`)
...
Release Please determines version bumps based on conventional commit types and the current version. Ark uses `bump-patch-for-minor-pre-major: true`, which affects how versions are bumped before reaching 1.0.0.

### For versions < 1.0.0 (current state)

Current version example: `0.1.63` {/* x-release-please-version */}

| Commit Type | Example | Version Bump | Result |
|-------------|---------|--------------|--------|
| `feat:` | `feat: add new executor` | Patch | `0.1.63` → `0.1.64` |
| `fix:` | `fix: resolve memory leak` | Patch | `0.1.63` → `0.1.64` |
| `feat!:` or `BREAKING CHANGE:` | `feat!: remove deprecated API` | Minor | `0.1.63` → `0.2.0` |
| Other (`docs:`, `chore:`, etc.) | `docs: update README` | None | Included in changelog only |

### For versions ≥ 1.0.0 (future state)

| Commit Type | Example | Version Bump | Result |
|-------------|---------|--------------|--------|
| `feat:` | `feat: add new executor` | Minor | `1.0.0` → `1.1.0` |
...
| ark-mcp | `ark-mcp` | `services/ark-mcp/ark-mcp/` | MCP server host |
| arkpy | `arkpy` | `services/arkpy/` | Python utilities |
| postgres-eventsink | `postgres-eventsink` | `services/postgres-eventsink/` | PostgreSQL event storage backend |
| autogen-execution-engine | `autogen-execution-engine` | `services/autogen-execution-engine/` | AutoGen executor implementation |
| a2agw-python | `a2agw-python` | `services/a2agw-python/` | Agent-to-Agent gateway (Python) |
...
**Python packages** (8 locations):
- `lib/ark-sdk/pyproject.toml`
- `lib/ark-sdk/gen_sdk/overlay/python/pyproject.toml`
- `services/arkpy/pyproject.toml`
- `services/ark-api/ark-api/pyproject.toml`
- `services/ark-mcp/ark-mcp/pyproject.toml`
- `services/postgres-eventsink/pyproject.toml`
- `services/autogen-execution-engine/pyproject.toml`
- `services/a2agw-python/pyproject.toml`
