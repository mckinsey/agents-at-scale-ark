...
7. **Response Generation**: Collects and formats the final response
   - Agent and team internals run on A2A-native messages, with compatibility conversion applied at the controller response boundary
...
2. Projects native A2A execution output to Query response payloads
3. Updates the Query status with results
4. Stores conversation in Memory (if configured), using native A2A writes when available and compat fallback otherwise
5. Marks the Query as completed
