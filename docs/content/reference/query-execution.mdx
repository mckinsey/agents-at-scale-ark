...
3. Routes recursively — members with their own execution engine go back through A2A; the rest run locally. A member call carries an explicit `target` in the query extension naming that member, because the query targets the team. Mixed teams — some members on a named engine, some not — are supported. Every member's output is appended to one transcript, delivered as structured messages to local members and as a `# <member>:`-attributed text transcript to named-engine members. A member on the built-in `a2a` engine contributes its output but receives only the query's original input, not the transcript — see [Mixed teams](/user-guide/teams#mixed-teams).
