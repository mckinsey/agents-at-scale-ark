...
Tool calls that fail with a transient error — HTTP 408, 425, 429, or 5xx, plus connection-level failures — are retried with exponential backoff and jitter. A `Retry-After` header from the server sets the minimum delay for the next attempt, capped at the maximum backoff. Non-transient errors (auth failures, validation errors, application-level tool errors) fail immediately.
...
Retried attempts are counted in the `ark_mcp_tool_call_retries_total{result, server}` metric, exposed on the completions executor's `/metrics` endpoint.
