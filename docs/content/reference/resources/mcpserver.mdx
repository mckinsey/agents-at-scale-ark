...
When Ark detects this, it fetches the RFC 9728 protected resource metadata, then the authorization server metadata using the well-known locations from the MCP authorization spec (RFC 8414 with path insertion, then OpenID Connect discovery), and populates `status.authorization`:
...
| `DiscoveryFailed` | Server returned 401 but no usable RFC 9728 metadata was found, or the authorization server advertised no `authorization_endpoint` and `token_endpoint`. Neither the CLI nor the dashboard can drive an OAuth flow against this server. Servers using `spec.authorization.clientCredentials` are exempt from the endpoint check. |
...
A successful flow writes the token material to the referenced Secret and the controller reconciles `status.authorization.state` to `Authorized`. ark-api does not record the identity of the caller who triggered the flow.

MCPServer tokens are a per-server singleton, only one user can hold an authorized session at a time. A future per-user-tokens capability will lift this limitation.
...
| `status.authorization.lastDiscovered` | Time the authorization block was last recomputed and persisted. Polls that leave the block unchanged do not advance it. |
