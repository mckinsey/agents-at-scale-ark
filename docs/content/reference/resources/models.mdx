...
1. **API Key (Bearer Token)** - Simpler authentication using an Amazon Bedrock API key
2. **Access Key / Secret Access Key** - Traditional AWS credentials

#### API Key Authentication (Recommended)

API key authentication is the simplest way to authenticate with AWS Bedrock. Generate an API key from the AWS Console and store it in a Kubernetes Secret.
...
type: Opaque
stringData:
  token: "your-bedrock-api-key-here"
```

For more information on generating Bedrock API keys, see the [AWS documentation](https://docs.aws.amazon.com/bedrock/latest/userguide/api-keys.html).

#### Access Key / Secret Access Key Authentication

For traditional AWS credential authentication:
