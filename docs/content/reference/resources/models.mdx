...
metadata:
  name: default
spec:
  # type defaults to 'completions' (chat completions) — the only value
  # accepted today, so it can be omitted.
...
metadata:
  name: claude-haiku
spec:
  # The AWS Bedrock provider with completions (chat) capability.
  provider: bedrock
  type: completions
  model:
