...
spec:
  ...
  retryPolicy:
    maxRetries: 3           # 0-10, default 0 (no retries)
...
status:
  ...
  # Number of retry attempts made (if retryPolicy configured)
  retryCount: 0
...
Configure automatic retries for transient failures like rate limits or network timeouts.
...
|-------|-------------|---------|
| `maxRetries` | Maximum retry attempts (0-10) | `0` (disabled) |
| `backoffPolicy` | Delay strategy between retries | `exponential` |
| `initialDelay` | Delay before first retry | `1s` |
| `maxDelay` | Maximum delay cap | `30s` |

### Backoff Policies

- **exponential**: Delay doubles each retry (1s → 2s → 4s → 8s...), capped at `maxDelay`
- **linear**: Delay increases by `initialDelay` each retry (1s → 2s → 3s → 4s...), capped at `maxDelay`
- **fixed**: Constant delay of `initialDelay` between all retries (1s → 1s → 1s...)

### Retry Behavior

Retries only occur for **transient errors** - temporary failures that may succeed on retry. Non-transient errors fail immediately without retry.

**Transient HTTP status codes** (following OpenAI SDK conventions):
...
**Status tracking:**
- `status.retryCount` tracks retry attempts made
- Query phase becomes `error` if all retries are exhausted or a non-transient error occurs

### Monitoring Retries

Check retry status with kubectl:

```bash
kubectl get query my-query -o jsonpath='{.status.retryCount}'
```

The `Retries` column is also visible in query listings:

```bash
kubectl get queries
# NAME        TYPE   PHASE   DURATION   RETRIES   AGE
# my-query    user   done    1.2s       2         5m
```
...
kind: Query
metadata:
  name: resilient-api-query
spec:
  input: "Fetch latest market data"
  target:
    type: agent
