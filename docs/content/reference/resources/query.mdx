...
For A2A agents specifically, timeouts follow a precedence hierarchy:

1. **Query.spec.timeout** (highest priority) - overrides all other timeouts
2. **A2AServer.spec.timeout** - default for that A2A server
3. **Deployment defaults** - see [Building A2A Servers](/developer-guide/building-a2a-servers#timeout-configuration)
