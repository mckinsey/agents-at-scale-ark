...
A `Query` is a request to run work on an Ark resource. The CRD ships an input plus exactly one of `target` (a single agent / team / model / tool) or `selector` (a label-based fan-out). For a task-oriented walkthrough see [User Guide → Run queries / chat with agents and teams](/user-guide/queries); this page is the field-by-field reference.
...
spec:
  ...
  # OR fan out via labels — matched resources each produce a response.
  # selector:
  #   matchLabels:
  #     role: reviewer
...
| `spec.selector` | `LabelSelector` | conditional | Label-based fan-out (alternative to `target`). Matched resources each produce a response. |
...
| `spec.sessionId` | string | no | Identifier for grouping queries in tracking / telemetry. **Not** the same as `conversationId` — `sessionId` is observability, `conversationId` is memory continuity. |
| `spec.conversationId` | string | no | Conversation threading identifier. Sent to execution engines as the A2A `contextId`. With `memory` set, ties subsequent queries to the same history; without `memory`, still flows through to engines that have their own session model. |
...
A query dispatches to exactly one target (set via `spec.target`) or fans out across many (set via `spec.selector`). Supported `type` values: `agent`, `team`, `model`, `tool`.
...
- Applies to all targets in a fan-out query.
...
| `status.response` | Single response object (singular — there is no `responses[]` array). For fan-out queries with a selector, the controller still publishes one composite response. |
