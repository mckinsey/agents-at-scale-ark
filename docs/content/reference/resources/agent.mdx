...
spec:
  ...
  # Config values for declarative components, that use declarative template-based execution engines (optional)
  # These are injected as ARK_CONFIG_* environment variables
  config:
    my-key: "my-value"
    my-feature-flag: "my-feature-flag-value"
...
status:
  ...
  # For template-based agents, the service address where the agent is accessible
  serviceAddress: "http://my-agent.default.svc.cluster.local:8080"
```
...
Agent using a custom container image (via ExecutionEngine template):
...
spec:
  executionEngine:
    name: researcher-template  # References an ExecutionEngine with source.image
  config:
    api_key: "sk-..."
...
The referenced ExecutionEngine:
...
spec:
  ...
  source:
    image: ghcr.io/your-org/researcher-agent:v1.0.0
  isAgentic: true
  description: "Research agent with web search capabilities"
```

When created, Ark deploys a dedicated pod running the specified image. Config values
are injected as environment variables (`ARK_CONFIG_API_KEY`, `ARK_CONFIG_MODEL_NAME`, etc.).
