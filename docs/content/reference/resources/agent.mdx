...
metadata:
  ...
  annotations:
    ...
    # Extensions discovered from the AgentCard capabilities (JSON array of URIs)
    ark.mckinsey.com/a2a-supported-extensions: '["https://ark.mckinsey.com/extensions/history/v1","https://ark.mckinsey.com/extensions/permissions/v1"]'
spec:
...
### A2A Supported Extensions Annotation

The `ark.mckinsey.com/a2a-supported-extensions` annotation controls which A2A extension metadata is included when Ark calls an A2A agent. The value is a JSON array of extension URIs:

```yaml
ark.mckinsey.com/a2a-supported-extensions: '["https://ark.mckinsey.com/extensions/history/v1"]'
```

**Runtime behavior:**

| Annotation state | Effect |
|---|---|
| **Absent** | Each extension uses its default policy. History is included; permissions are excluded. |
| **Present with URIs** | Only declared URIs receive metadata. Extensions not listed are excluded. |
| **Present but empty `[]`** | All extensions are excluded. |

This annotation is populated automatically by the [A2AServer](/reference/resources/a2aserver) controller from the AgentCard's `capabilities.extensions`. It can also be set manually on an Agent for agents not managed by an A2AServer.
