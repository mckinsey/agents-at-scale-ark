...
When creating a query in the dashboard it is also possible to specify the memory resource. The dashboard chat window keeps its transcript client-side, and every turn also carries a `conversationId`: the chat does not set one, but the A2A layer generates a context ID at dispatch and the executor uses that, so a query has one even on a brand-new conversation in a namespace with no memory backend at all.
...
- `MemoryUnavailable` requires a `conversationId`, since it means "continuity was expected and not delivered". Every dashboard chat turn has one — the A2A layer generates a context ID at dispatch even when the chat supplies none — so this covers a namespace that never had a memory backend as well as one that lost it.
