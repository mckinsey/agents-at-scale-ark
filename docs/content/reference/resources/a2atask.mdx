...
- Runtime compatibility baseline is **A2A v0.3**.
- Current persisted part examples use v0.3-style `kind` parts (`text`, `file`, `data`).
- V1 RC adoption is adapter-based and additive; see the mapping section in this document for part-shape translation.
...
- **protocolState** (A2A protocol): Last observed protocol state (`submitted`, `working`, `input-required`, `auth-required`, `completed`, `canceled`, `failed`, `rejected`)
- **contextId** (A2A protocol): Conversation context identifier
...
- **artifacts** (A2A protocol projection): Array of task artifacts produced during execution, each with `artifactId`
- **history** (A2A protocol projection): Message history with `messageId` for deduplication
- **protocolMetadata** (A2A protocol): Additional key-value metadata captured from upstream task payloads
- **lastStatusMessage** (A2A protocol): Last status message projection from task updates
- **lastStatusTimestamp** (A2A protocol): RFC3339 status timestamp from latest protocol update
...
### v0.3 -> V1 Part Mapping

Current runtime payloads and this CRD surface are v0.3-oriented (`kind` discriminator). For V1 RC migration, adapters map shapes as follows:

| v0.3-compatible part shape | V1 RC-oriented shape |
| --- | --- |
| `{"kind":"text","text":"hello"}` | `{"text":"hello"}` |
| `{"kind":"file","file":{"uri":"https://example.com/a.png"},"mimeType":"image/png"}` | `{"url":"https://example.com/a.png","mediaType":"image/png"}` |
| `{"kind":"file","file":{"bytes":"<base64>"},"mimeType":"application/pdf"}` | `{"raw":"<base64>","mediaType":"application/pdf"}` |
| `{"kind":"data","data":{"k":"v"}}` | `{"data":{"k":"v"}}` |

During migration windows, readers should tolerate both shapes; emitters should choose one shape intentionally per configured wire version.
...
metadata:
  name: weather-agent
  annotations:
    ark.mckinsey.com/a2a-server-name: langchain-agents
spec:
