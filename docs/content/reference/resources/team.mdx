...
spec:
  ...
  # Team-level prompt - context/guidelines for team execution
  prompt: "Work together to analyze and write comprehensive reports"

  # Team leader - designated leader (must be one of the team members)
  leader: researcher
...
The `prompt` field provides team-level context and guidelines for team execution. This prompt is metadata that describes the team's purpose and collaboration approach.
...
The `leader` field designates a team leader from the team members. The leader must be one of the members listed in the `members` array.

```yaml
leader: researcher  # Must be one of the team members
```

The leader designation is metadata and does not affect execution order or logic.
