...
Two strategies are accepted on create/update but get rewritten by the mutating webhook (`ark/internal/validation/defaults.go`) and removed in `v1.0.0`. Both rewrites stamp a `ark.mckinsey.com/migration-warning/<key>` annotation on the team for visibility.
...
`maxTurns` reached during execution emits a `TeamMaxTurnsReached` warning event on the Query (not on the Team) and completes the query successfully with the responses collected so far — it's not an error.

## Related

- [User Guide → Teams](/user-guide/teams) — task-oriented walkthrough.
- [User Guide → Samples → Team Strategies](/user-guide/samples/teams/team-strategies) — runnable strategy examples.
