...
catalogue is shared by every user of that namespace rather than living in each
...
| --- | --- | --- |
| `url` | yes | Absolute `https` URL of a `marketplace.json` manifest. |
...
The items aggregator fetches each source's `marketplace.json` concurrently
