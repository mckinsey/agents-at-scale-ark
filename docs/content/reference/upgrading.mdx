...
Version-specific upgrade instructions for breaking changes.
...
0. The `v0.1.x` version line is the initial API unstable development phase of a project, breaking changes can occur on new releases.
1. From `v0.2.x` onwards, breaking changes are fully documenting in this guide, without migration instructions.
2. Patch releases (e.g. `v0.2.1` to `v0.2.2` or `v0.2.x`) do not introduce breaking changes, and should have no special instructions to follow.
3. Minor releases (e.g. `v0.2.2` to `v0.3.0`) introduce small changes. Minor workaround might be needed, and the [changelog](https://github.com/mckinsey/agents-at-scale-ark/blob/main/.github/CHANGELOG.md) should be checked.
4. Minor releases prior to `v0.1.0` (e.g. `v0.3.0` to `v0.4.0`) may introduce backwards incompatible behavior changes, which are documented in this guide with migration instructions.
