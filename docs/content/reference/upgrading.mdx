...
`clusterworkflowtemplates` are cluster-scoped, so they are only included in a cluster-wide export. When you scope the export to a namespace with `-n`, they are skipped — a namespace-scoped export captures that namespace's resources, not the cluster-wide templates shared across tenants.
