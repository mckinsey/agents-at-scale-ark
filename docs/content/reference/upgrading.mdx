...
Prior to `v0.1.49`, the Ark controller automatically appended `/mcp` or `/sse` to MCP server addresses based on the transport type. This prevented users from connecting to MCP servers with custom endpoint paths.

From version `v0.1.49`, MCP server addresses are used exactly as specified. You must include the full path in your MCPServer configuration.
