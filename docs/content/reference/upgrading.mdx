...
Delegated tool execution now follows A2A task lifecycle semantics end-to-end.

**Previous behavior**
- Local execution loop appended synthetic tool-result messages to continue the next model turn.
...
This is a **breaking** change for consumers parsing delegated streams/messages as tool-message records.
...
3. Stop depending on synthetic tool-result messages being present in `A2AMessages`.
4. Stop depending on OpenAI chunk payloads appearing in delegated streams.
