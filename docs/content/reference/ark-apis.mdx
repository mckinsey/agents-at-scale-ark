...
The `ark-api` service exposes a REST interface for executing queries and managing resources. This page covers the base URL, authentication, conventions, the query workflow, and a reference of every endpoint group. Trace ingestion and retrieval are served separately by the broker — see [Traces](/reference/traces).
...
- **Local dev**: `kubectl port-forward svc/ark-api 8000:80`, then use `http://localhost:8000`. Or run `ark routes` / `make routes` to get a gateway URL such as `ark-api.default.127.0.0.1.nip.io:8000`.

Open the Swagger UI at `/docs` to browse and try every endpoint:

![Ark API Swagger UI](/ark-apis.png)

You can also reach it from the dashboard — the **APIs** link at the bottom left:
...
Telemetry the broker captures for a namespace. (Trace ingestion and the OTLP endpoint are documented under [Traces](/reference/traces).)
