...
- V1 RC support is adapter-scoped and migration-oriented.
...
| Area | Status |
| --- | --- |
| Native A2A task streaming path | Implemented in experimental A2A mode |
| Task-first delegated streaming + correlation extension | Implemented |
| Experimental V1 implementation and rollout docs | Published |
| OpenAI Responses API adapter | Planned (tracked in issue #965) |
| Default non-experimental compat path | Unchanged |

This means current behavior remains:
- A2A-native task streaming is available in experimental mode.
- Responses API support should be implemented as an edge adapter, not as internal transport replacement.
