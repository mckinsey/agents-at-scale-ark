...
#### A2A Gateway Not Available (Deprecated - Now Part of ark-api)

> **Note:** The A2A Gateway functionality has been integrated into `ark-api`. This section is kept for historical reference.

**Error or Symptom**

```bash
# Old issue: ark-api-a2a not installed
# New: A2A Gateway is now part of ark-api
```

**Solution**

A2A Gateway is now part of `ark-api`. Install `ark-api` and the A2A endpoints will be available at:
- `GET /agents` - List agents
- `GET /agent/{name}/.well-known/agent.json` - Agent cards
- `GET /agent/{name}/*` - A2A protocol routes

If A2A endpoints are not working, verify:
1. `ark-api` is installed and running
2. Check logs: `kubectl logs -n default deployment/ark-api | grep "A2A Gateway"`
3. Verify environment variables are set in `ark-api` deployment
