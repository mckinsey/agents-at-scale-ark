...
The **messages** and **operation events** stores can each opt in to a Postgres backend (`backends.message: postgres` and `backends.event: postgres`, sharing one database) so that data survives pod restarts, and the **chunks** store can opt in to Redis (`backends.chunk: redis`). These add durability (and, for chunks on Redis, cross-replica live streaming); they do not by themselves make the broker horizontally scalable, since traces and sessions still remain in-memory per replica.
