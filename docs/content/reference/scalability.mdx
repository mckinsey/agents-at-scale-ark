...
Ark's control plane runs on Kubernetes and inherits its scaling primitives. The controller is the single stateful coordination point; all other services (API, dashboard, broker) are stateless and can be horizontally scaled.
...
The API, dashboard, and broker are stateless and can be scaled horizontally by increasing replica counts in their Helm values. The broker stores messages, streaming chunks, and traces in memory by default, so each replica maintains its own state — consider enabling file-based persistence or a sticky session strategy if scaling the broker beyond a single replica.
...
Executors run as separate deployments and scale independently from the Ark control plane. Each executor type can define its own HPA, resource limits, and replica strategy. The MCP server template includes an optional HorizontalPodAutoscaler configuration that scales on CPU utilization.
...
- **Conversation history** — If the broker has persistence enabled, or if you use an external memory backend, storage grows with conversation volume. Plan retention and cleanup accordingly.
...
3. **Scale stateless services** — Increase replica counts for the API, dashboard, and executors as needed.
