...
│         │       kube-apiserver         │                                │
│         │                              │                                │
│         │   All resources handled      │                                │
│         │   by single API server       │                                │
│         └──────────────┬───────────────┘                                │
│                        │                                                │
│                        ▼                                                │
│         ┌──────────────────────────────┐      ┌─────────────────────┐   │
│         │           etcd               │      │    Ark Controller   │   │
│         │                              │◄────►│                     │   │
│         │   • pods, services           │      │  • Reconciliation   │   │
│         │   • deployments              │      │  • Query execution  │   │
│         │   • queries, agents          │      │  • Webhooks         │   │
│         │   • models, teams            │      └─────────────────────┘   │
│         └──────────────────────────────┘                                │
└─────────────────────────────────────────────────────────────────────────┘
```
...
│         │       kube-apiserver         │                                │
│         │                              │                                │
│         │   Routes ark.mckinsey.com    │                                │
│         │   to aggregated API server   │                                │
│         └───────┬──────────────┬───────┘                                │
│                 │              │                                        │
│     ┌───────────┘              └───────────┐                            │
│     ▼                                      ▼                            │
...
│  │                  │           │   (aggregated)       │                │
│  │  • pods          │           │                      │                │
│  │  • services      │           │  • queries, agents   │                │
│  │  • deployments   │           │  • models, teams     │                │
│  │  • configmaps    │           │  • memories          │                │
│  └──────────────────┘           └──────────┬───────────┘                │
│                                            │                            │
│                                            ▼                            │
...
│                                 │     PostgreSQL       │                │
│  ┌─────────────────────┐        │                      │                │
│  │   Ark Controller    │        │  • Keyset pagination │                │
│  │                     │◄──────►│  • LISTEN/NOTIFY     │                │
│  │  • Reconciliation   │        │  • SKIP LOCKED queue │                │
│  │  • Query workers    │        │  • Composite indexes │                │
│  │  • Webhooks         │        └──────────────────────┘                │
│  └─────────────────────┘                                                │
└─────────────────────────────────────────────────────────────────────────┘
```
