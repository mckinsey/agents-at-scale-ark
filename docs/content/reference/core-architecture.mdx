...
A running Ark install is made up of a control plane (the controller and its webhooks) plus a handful of supporting services. Clients — `kubectl`, the dashboard, or the REST API — create resources through the Kubernetes API; the controller reconciles them and dispatches queries to an execution engine over the A2A protocol.
