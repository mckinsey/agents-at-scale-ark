...
The two backends hold independent copies of the resources, so switching does not move data. See [Migrating between storage backends](/operations-guide/postgres-storage-backend#migrating-between-storage-backends) before changing the backend on an install that holds resources you care about.
