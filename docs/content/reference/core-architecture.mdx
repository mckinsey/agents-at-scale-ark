...
2. Delete etcd-mode CRDs (`*.ark.mckinsey.com`) — the aggregated API server replaces them
3. For helm-managed installations, re-label APIServices with `app.kubernetes.io/managed-by=Helm` and annotate with the helm release name so the upgrade can adopt them
4. Upgrade the controller with `storage.backend=postgresql` and the PostgreSQL connection settings
5. Migrate existing resources using `kubectl get -o yaml | kubectl apply -f -`
6. Verify watch events and controller reconciliation
