...
At a high level: you create ARK resources through the Kubernetes API (via `kubectl`, the `ark` CLI, the dashboard, or `ark-api`). The **Ark controller** reconciles them, and when a `Query` runs it delegates execution over the A2A protocol to the **completions engine** (the default executor), which calls model providers, MCP tools, and other A2A agents, streaming results through `ark-broker`.

```mermaid
flowchart TB
    users["Users and clients<br/>kubectl · ark CLI · browser · REST/A2A · MCP clients"]

    subgraph cluster["Kubernetes cluster"]
        api["Kubernetes API server"]
        store[("ARK resources — etcd or PostgreSQL<br/>Agent · Team · Model · Query · Tool<br/>MCPServer · A2AServer · A2ATask<br/>Memory · ExecutionEngine · ArkConfig")]

        subgraph control["ark-system"]
            ctrl["Ark controller<br/>reconcile + webhooks"]
            engine["Completions engine<br/>default executor (A2A)"]
        end

        subgraph svcs["Ark services"]
            arkapi["ark-api<br/>REST + A2A gateway"]
            dash["ark-dashboard"]
    ...
    subgraph external["External / pluggable"]
        models["Model providers<br/>OpenAI · Azure · Anthropic · Bedrock"]
        tools["MCP servers<br/>(agent tools)"]
        a2a["A2A servers and custom execution<br/>engines (e.g. LangChain — marketplace)"]
    end
    ...
    mcp --> api
    api <--> store
    api <-->|"watch + status"| ctrl
    ctrl -->|"A2A SendMessage"| engine
    ctrl -.->|"custom engine"| a2a
    engine --> models
    engine --> tools
    engine --> a2a
    engine -->|"stream"| broker
    arkapi -->|"stream"| broker
```
...
- **ArkConfig**: Namespace-scoped configuration for ARK defaults.
