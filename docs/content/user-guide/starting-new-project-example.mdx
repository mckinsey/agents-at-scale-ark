...
kubectl get query test-password-reset -o jsonpath='{.status.responses[0].content}' | jq -r
```
...
kubectl get query test-billing -o jsonpath='{.status.responses[0].content}' | jq -r
```
...
## Step 11: Create a Team Workflow
...
3. **Deployed** the agent to ARK using `make deploy` or `kubectl`
4. **Tested** with queries and viewed responses
5. **Monitored** execution via kubectl and dashboard
6. **Built** multi-agent workflows with teams
