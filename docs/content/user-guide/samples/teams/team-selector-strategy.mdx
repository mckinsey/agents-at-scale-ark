...
3. **Participant Selection**: Agent uses the `select-next-participant` tool to pick a team member
...
Custom prompts **must** instruct the agent to use the `select-next-participant` tool. Without this, the selector agent will not know how to make a structured selection and the team will fail.
...
spec:
  selector:
    ...
    selectorPrompt: |
      ...
      Use the select-next-participant tool to choose the next participant.
```
...
spec:
  ...
  selector:
    ...
    selectorPrompt: |
      ...
      Use the select-next-participant tool to make your selection.
```
...
The selector agent is given a `select-next-participant` tool whose `name` parameter is an enum of valid candidates. For graph-constrained teams, the enum only includes members reachable from the current node. This ensures the selector can only pick a valid next participant.
