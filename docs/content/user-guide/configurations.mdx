...
With [impersonation](/developer-guide/authentication) enabled, the Ark API acts as the logged-in user, so that user needs the ConfigMap permissions themselves. The chart creates a [ClusterRole](https://github.com/mckinsey/agents-at-scale-ark/blob/main/services/ark-api/chart/templates/configmap-editor-role.yaml) for this, suffixed with the namespace Ark is installed in, and leaves it unbound. Bind it where the configurations live:
