...
- An **external member** gets the transcript folded into the prompt body with `# <member>:` speaker attribution, because the receiving engine assembles its own prompt. Tool-result messages don't survive that flattening, so if an internal member uses tools, an external member downstream sees the assistant turns but not the tool output.
