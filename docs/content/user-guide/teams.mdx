...
A query that targets an `a2a` agent directly still sends only that query's input, leaving the conversation to the remote server's own A2A context. Only team-member turns carry the transcript.
