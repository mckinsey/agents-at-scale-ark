...
The controller validates that every member resolves and that the strategy's cross-field rules hold (more on those below).
...
A team cannot reference itself. Members can mix execution engines — see [Mixed teams](#mixed-teams).

## Mixed teams

A team can contain both internal agents (default completions executor) and external agents on a named `executionEngine` (e.g. Claude Agent SDK, LangChain):
...
spec:
  strategy: sequential
  members:
    - name: claude-coder      # spec.executionEngine.name: claude-agent-sdk
      type: agent
    - name: local-reviewer    # spec.modelRef, no executionEngine
      type: agent
```

Every member sees the same transcript — each member's output is appended to the conversation and passed to the next one — but the two kinds receive it in different shapes:

- An **internal member** gets its own system prompt plus the transcript as structured messages.
- An **external member** — on a named engine or on the built-in **`a2a`** engine — gets the transcript folded into the prompt body with `# <member>:` speaker attribution, because the receiving engine assembles its own prompt. Tool results are rendered as `# tool:` turns, but the tool call that produced them — its name and arguments — is not carried across.

A query that targets an `a2a` agent directly still sends only that query's input, leaving the conversation to the remote server — see [A2AServer → Conversation context](/reference/resources/a2aserver#conversation-context). Only team-member turns carry the transcript.

Conversation state is scoped differently for each of the three: internal members share the parent query's memory; each **named-engine** member gets its own derived `conversationId`, so an executor that keys state on it won't merge members; an **`a2a`** member keeps the query's A2A `contextID`, so the remote server sees one conversation for the whole query.
...
The `Available` condition flips to `False` when the team is misconfigured — `kubectl describe` shows the underlying admission error (missing member, missing selector agent, missing `maxTurns`, etc.).
