...
`round-robin` and standalone `graph` are deprecated and **migrated automatically by the mutating webhook** when you apply them — `round-robin` becomes `sequential + loops: true`; `graph` becomes `sequential` (the graph edges are discarded). Both stamp a `ark.mckinsey.com/migration-warning/...` annotation on the Team. New configs should use the strategies in the table.
...
kubectl get query team-q1 -o jsonpath='{.status.responses[*].content}'
```

In the dashboard, opening a team also opens an embedded chat panel on the right — type into it to query the team interactively. The chat panel has a **Stop** button to cancel a running conversation.
...
A Team can be wired into an agent the same way an MCP server tool would be — the agent sees the team as a callable tool with the team's `description` as the tool description:
...
When the agent calls the tool, the team executes a normal run and returns its final message back to the agent.
...
**`AVAILABLE: False` after apply.** Check the `Available` condition:

```bash
kubectl get team my-team -o jsonpath='{.status.conditions[?(@.type=="Available")].message}'
```

Common causes the webhook surfaces:

- **`selector strategy requires maxTurns`** — add `spec.maxTurns: <n>` (e.g. 10).
- **`selector strategy requires selector.agent`** — every selector team needs a `selector.agent` pointing at an existing Agent in the same namespace.
...
- **`graph edge N: 'from' member 'X' not found in team members`** — every `from`/`to` in `graph.edges` must reference a member of the team.
...
- **`team member N: agent 'X' not found in namespace 'Y'`** — apply the referenced Agent first.

**Selector team terminates immediately with no response.** The selector agent returned text instead of calling the `select-next-speaker` tool. Either reset the prompt (the dashboard has a button) or make sure your custom prompt instructs the agent to use the tool. The team will surface a warning event with the offending output.

**`TeamMaxTurnsReached` event but the work isn't done.** Increase `maxTurns`, or give a member the `terminate` built-in tool so the team can exit cleanly earlier.
