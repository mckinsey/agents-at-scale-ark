...
| **`inline`** | A short script stored in the tool itself | You author it, Ark runs it. Off by default and needs an administrator grant — see [Inline tools](#inline-tools). |
...
| **Type** | `HTTP`, `MCP`, `Agent`, `Team`, or `Inline`. (Built-in tools aren't created here — they ship with the install.) |
...
An inline tool carries its own script instead of pointing at something you host:
...
spec:
  ...
  inputSchema:
    ...
    required: ["text"]
  inline:
    language: python      # bash | python | node | ts
    source: |
      import json, sys
      print(len(json.loads(sys.argv[1])["text"].split()))
```

In the dashboard, pick **Inline** in the Add Tool form, choose a language, and paste the source. The tools list shows a language badge.

For runnable examples — a Python CSV summariser, a bash text counter, and an agent that uses both — apply `samples/tools/inline-tools.yaml`. Both pass their data in as arguments, because a runner has no file mounts and no network access.

Two things to know before you try it:

- **Attach it like any other tool**, with `type: inline` in the agent's `spec.tools`. The default execution engine calls it over MCP once the tool reports `Available`; agents on an external execution engine can't call inline tools yet.
- **Authoring is off by default and permission-gated.** An administrator must enable `inlineTools.enabled` in the Ark chart (and the ark-apiserver chart on the PostgreSQL backend), then grant you the `inline-tool-author-role` in your namespace:
...
Without both, the write is denied — `tools/create` on its own never permits storing executable code. Ark stamps the author and time on every inline write as `ark.mckinsey.com/inline-authored-by` and `ark.mckinsey.com/inline-authored-at`. Source is limited to 65536 bytes; use an MCP server for anything bigger. Full details: [Reference → Tools → Inline tools](/reference/resources/tools#inline-tools).
...
The dashboard's **Add Tool** form covers name, type (`HTTP` / `MCP` / `Agent` / `Team` / `Inline`), description, input schema, and annotations — plus the type-specific target (HTTP URL, the agent/team picker, or the inline source and its language). These tool features are only configurable via the `Tool` YAML for now:
...
- **`builtin` tools.** The form's type dropdown is `HTTP` / `MCP` / `Agent` / `Team` / `Inline` only. Built-in tools (`terminate`, `noop`) aren't created from the UI — they're seeded by the install and referenced by name.
