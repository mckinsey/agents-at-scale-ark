...
| **`inline`** | A short script stored in the tool itself | You author it. Off by default and needs an administrator grant — see [Inline tools](#inline-tools). |
...
- **Inline tools are not callable yet.** No runner ships in this release, so an authored inline tool stays `Pending` with reason `RuntimeNotInstalled`. The dashboard says so on the tool.
