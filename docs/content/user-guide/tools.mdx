...
You can also **author a built-in `Tool` yourself** — for example to re-create one in a namespace where the chart didn't seed it, or to customise its `description` / `inputSchema`:

```yaml
apiVersion: ark.mckinsey.com/v1alpha1
kind: Tool
metadata:
  name: terminate          # must be one of the supported built-ins
spec:
  type: builtin
  ...
  inputSchema:
    ...
    properties:
      response:
        type: string
        description: The final response to send before terminating
    required: ["response"]
  builtin:
    name: terminate
```

The catch: the set of built-in *implementations* is fixed in Ark's code, and both the admission webhook and the executor key on the **resource name** — so the `Tool`'s `metadata.name` (and `spec.builtin.name`) must be `terminate` or `noop`. You can't invent a new built-in behaviour by authoring a `Tool`; that requires a code change to Ark. What you *can* do is own the resource definition (schema, description, which namespace it lives in).
...
- **`builtin` tools.** The form's type dropdown is `HTTP` / `MCP` / `Agent` / `Team` only — built-in tools aren't creatable from the UI (they're seeded by the install, or authored in YAML as shown above).
