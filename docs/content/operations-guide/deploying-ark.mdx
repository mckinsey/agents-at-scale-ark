...
rbac:
  ...
  impersonation:
    ...
    #   - system:serviceaccount:tenant-1:ark-tenant-sa
    #   - system:serviceaccount:tenant-2:ark-tenant-sa
...
helm upgrade --install ark-controller oci://ghcr.io/mckinsey/agents-at-scale-ark/charts/ark-controller \
  ...
  --set-string rbac.impersonation.serviceAccounts='{system:serviceaccount:default:default,system:serviceaccount:tenant-1:ark-tenant-sa,system:serviceaccount:production:prod-sa}'
```
...
```bash
# Note: Must include full namespace path in service account names
helm upgrade ark-controller oci://ghcr.io/mckinsey/agents-at-scale-ark/charts/ark-controller \
  -n ark-system \
  --set rbac.impersonation.allowAll=false \
  --set-string rbac.impersonation.serviceAccounts='{system:serviceaccount:default:default,system:serviceaccount:tenant-1:ark-tenant-sa,system:serviceaccount:tenant-2:ark-tenant-sa}'
```
