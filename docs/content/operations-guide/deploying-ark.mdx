...
When provisioning new tenant namespaces, ARK provides template RBAC resources (`ark-tenant-role` and `ark-tenant-binding`) that grant access to essential Kubernetes resources (pods, services, deployments, secrets, configmaps) and all ARK resources (agents, models, queries, teams, tools, etc) within that namespace only.

These templates provide namespace-scoped permissions that prevent cross-tenant access. However, the included permissions are broad and suitable primarily for development environments. For production multi-tenant scenarios, cluster administrators should customize the role permissions based on the principle of least privilege for each tenant's specific requirements.

### Setting Up Tenant Namespaces

Create the namespace and service account (if using custom service accounts):
...
Apply the ARK tenant RBAC:

```bash
# Apply the tenant role
kubectl apply -f ark/config/rbac/ark_tenant_role.yaml -n tenant-1

# Create role binding for the service account
kubectl create rolebinding ark-tenant-binding \
  --role=ark-tenant-role \
  --serviceaccount=tenant-1:tenant-1-sa \
  -n tenant-1
```
...
metadata:
  ...
  namespace: tenant-1
spec:
  serviceAccount: tenant-1-sa  # Specify the service account
