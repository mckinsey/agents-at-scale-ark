...
For multi-tenant deployments, Ark uses Kubernetes namespaces as tenant boundaries with RBAC for isolation and cross-namespace access. See [Tenant and Namespace Management](./tenant-namespace-management) for setup instructions, RBAC tiers, and dashboard namespace configuration.
