...
The out-of-the-box installation gives the `ark-controller` permissions to impersonate the `default` and `ark-tenant-sa` service accounts. This can be configured in the Helm chart to support multi-tenancy:
...
rbac:
  enable: true
  impersonation:
    # Option 1: Allow all service accounts (less secure, more flexible)
    allowAll: true

    # Option 2: Explicitly list allowed accounts (more secure, recommended)
    # allowAll: false
    # serviceAccounts:
    #   - default
    #   - ark-tenant-sa
...
The ARK controller needs permission to impersonate tenant service accounts. Choose one approach:

**Option 1: Allow all service accounts (simplest for development)**

```bash
helm upgrade ark-controller ./ark/dist/chart --set rbac.impersonation.allowAll=true
```

**Option 2: Use standard naming convention**

If all tenants use `ark-tenant-sa` as their service account name (the default), no additional configuration is needed.

**Option 3: Explicitly list service accounts (most secure)**
