...
The `ark` CLI is the fastest way to install Ark on any cluster — `npm install -g @agents-at-scale/ark`, then `ark install` runs an interactive install of the core controller, its dependencies (cert-manager, Gateway API), and optional services.

The [Quickstart](/quickstart) covers that flow end to end — prerequisites, install, and verifying with `ark status` — and is kept current and tested, so it isn't repeated here. This page focuses on what the quickstart doesn't: Helm and remote-cluster deployment, storage backends, private registries, CI/CD RBAC, and multi-tenant operation.
