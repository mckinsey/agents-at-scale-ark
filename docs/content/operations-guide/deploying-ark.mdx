...
For the standard install — `npm install -g @agents-at-scale/ark`, then `ark install` — follow the [Quickstart](/quickstart), which is kept current and tested. This page covers what it doesn't: Helm and remote-cluster deployment, storage backends, private registries, CI/CD RBAC, and multi-tenant operation.
