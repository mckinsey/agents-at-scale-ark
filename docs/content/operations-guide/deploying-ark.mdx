...
rbac:
  ...
  impersonation:
    ...
    # Option 1: Allow impersonation of ANY service account (development/testing)
    allowAll: true

    # Option 2: Allow specific service accounts only (production)
    # When allowAll is true, serviceAccounts list is ignored
    # allowAll: false
    # serviceAccounts:
    #   - default
    #   - ark-tenant-sa
...
If the Ark controller needs specific permissions to impersonate tenant service accounts (see [The Ark Controller](#the-ark-controller) when `rbac.impersonation.allowAll` is `false`), then the new service account should be added to the list of allowed impersonation accounts:
