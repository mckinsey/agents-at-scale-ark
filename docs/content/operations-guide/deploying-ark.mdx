...
For the standard install — `npm install -g @agents-at-scale/ark`, then `ark install` — follow the [Quickstart](/quickstart), which is kept current and tested. This page covers: Helm and remote-cluster deployment, storage backends, private registries, CI/CD RBAC, and multi-tenant operation.
...
The [Tenant and Namespace Management](/operations-guide/tenant-namespace-management) guide is the canonical reference for multi-tenancy — CLI-based setup (`ark install ark-tenant`), cross-tenant access via RBAC, broker storage isolation, and the development workflow.
