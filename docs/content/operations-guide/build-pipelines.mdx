...
1. **Continuous Commit Analysis** (On every push to main)
   ...
   - Determines version bump based on commit types and current version:
     - For versions < 0.1.68 (current: 0.1.67): {/* x-release-please-version */}
...
3. **RC Release Activation** (When an RC Release PR is merged)
   - The CI/CD pipeline re-runs fully and creates a GitHub release with the new RC version tag
   ...
   - Sets `release_created` output to trigger downstream jobs (in `.github/workflows/cicd.yaml`):
      ...
      - **run-deploy-workflow**: Triggers the Deploy workflow (`deploy.yml`), which on an RC publishes multi-arch containers to GHCR and Helm charts to the OCI registry
   - RC releases do not update the container `latest` tag or publish the npm CLI, PyPI SDK, or docs to GitHub Pages — those are stable-only

4. **Stable Promotion** (Custom flow, not Release Please)
   - Unlike the automated RC flow above, an operator manually triggers the "Prepare Release" workflow (`prepare-release.yaml`) via `workflow_dispatch`, optionally passing the `rc_tag` to promote (blank uses the latest RC)
   - The workflow strips `-rc` across every version file, bumps the release-please manifest, records the promoted RC in `.releasing`, and opens a reviewed `release: vX.Y.Z` PR
   - Merging that PR runs `release-on-merge.yaml`, which tags the stable version, rebuilds artifacts **from the tested RC commit**, and triggers the Deploy workflow (`deploy.yml`) to publish containers, Helm charts, npm CLI, PyPI SDK, and docs to GitHub Pages, then runs installation verification
   - See [Release Process → Stable Release Cycle](/reference/release-process#stable-release-cycle) for the full flow. The older `promote-to-stable.yaml` + Release Please promotion path is retained as a fallback.
