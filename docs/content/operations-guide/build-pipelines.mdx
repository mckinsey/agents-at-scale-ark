...
3. **RC Release Activation** (When an RC Release PR is merged)
   ...
   - RC releases are not published to public registries

4. **Stable Promotion** (Custom flow, not Release Please)
   - A tested RC is promoted to stable via the "Prepare Release" workflow (`prepare-release.yaml`), which opens a reviewed `release: vX.Y.Z` PR
