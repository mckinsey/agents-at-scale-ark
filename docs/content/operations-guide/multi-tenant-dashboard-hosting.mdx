...
- For full cross-tenant isolation, register a distinct OIDC client per tenant and set `OIDC_APPLICATION_ID` to that tenant's client ID on its ark-api. The JWT's `aud` claim is then checked per tenant and tokens from one tenant are rejected at another. `OIDC_APPLICATION_ID` also accepts a comma-separated list (e.g. `app-a,app-b`) when a single ark-api must trust more than one audience — a token passes if its `aud` matches any listed value.
...
metadata:
  ...
  annotations:
    # Required for SSO dashboards. After login the dashboard emits a large
    # response header (the chunked NextAuth session Set-Cookie); nginx-ingress's
    # default proxy buffer (4k/8k) is too small and the tenant returns 502
    # "upstream sent too big header" on the post-login request. 16k is enough
    # for a chunked session cookie.
...
- **Login hub redirects back to the hub (or root) instead of the tenant after sign-in** — when a separate `ark-landing-page` hub signs users in on behalf of tenant dashboards on a different origin, set `AUTH_ALLOWED_CALLBACK_ORIGINS` on the hub to the comma-separated list of tenant origins (e.g. `https://tenant-a.example.com,https://tenant-b.example.com`). Auth.js only honours same-origin callback URLs; unlisted origins silently fall back to the hub's own `BASE_URL`, so the post-login redirect to the tenant is dropped.
- **Tenant returns 502 right after login (works for `curl` but not the browser)** — nginx logs `upstream sent too big header while reading response header from upstream`. The post-login request carries the session cookie and the dashboard responds with the large chunked NextAuth `Set-Cookie`, which overflows nginx-ingress's default proxy buffer. Add `nginx.ingress.kubernetes.io/proxy-buffer-size: "16k"` to the Ingress (already included in the example above and in `values-multi-tenant.example.yaml`).
