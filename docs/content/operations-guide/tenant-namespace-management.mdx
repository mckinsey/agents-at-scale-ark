...
A namespace without a broker silently loses conversation history: queries succeed, but the assistant never recalls a previous turn. To make that failure loud instead, `ark-tenant` refuses to install into a namespace with no broker Service — whether the release comes from `ark install ark-tenant` (unless `--no-deps` is passed) or a direct `helm install ark-tenant`. An existing tenant running `helm upgrade` is never blocked by this check. Pass `--set memory.requireBroker=false` to provision a tenant without a broker anyway.
