...
Storage is not the whole boundary. The broker authenticates no route, so any pod that can reach its Service can read and write another tenant's messages, events, and sessions whatever the Kubernetes RBAC says. Set `networkPolicy.enabled=true` on the broker chart to restrict ingress to the broker's own namespace plus the namespaces listed in `networkPolicy.allowNamespaces` — `ark-system` in practice, since the controller and the completions executor write from there while the broker runs in the tenant namespace. `allowNamespaces` admits whole namespaces rather than workloads, so listing `ark-system` opens the broker to every pod running there: the policy bounds reach, it does not identify the caller. The two policies in the repo are alternatives, not layers. A tenant that enables `networkPolicy.enabled` on `charts/ark-tenant` gets a namespace-wide policy that already covers its broker pods, on every port; the broker chart's own policy is for standalone installs, where it also scopes the rule to the broker's pod port. Enabling both buys nothing, because policies selecting the same pod combine as a union and the wider rule decides. Both are off by default, and enforcement is the CNI's job — a cluster whose CNI ignores NetworkPolicy accepts the object and restricts nothing.
