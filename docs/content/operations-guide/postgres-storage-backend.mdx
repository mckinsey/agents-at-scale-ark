...
`require` encrypts the connection but does not verify the server's identity. `verify-ca` and `verify-full` additionally validate the server certificate. Against a publicly-trusted managed database (RDS, Cloud SQL) they work with no extra configuration because the controller image trusts the system CA bundle. For a private CA, self-signed certificate, or mutual TLS, mount the PEM files via a Secret on the apiserver chart:
