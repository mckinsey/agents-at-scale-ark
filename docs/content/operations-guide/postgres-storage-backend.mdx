...
Switching backends does not move data. etcd and PostgreSQL hold independent copies of the `ark.mckinsey.com` resources and neither backend can see the other's. Export before the switch, import after.

Both directions use the same four steps, with a different switch in step 2. The procedure is re-runnable: the export files carry plain desired state, and `ark import --upsert` can be repeated against a partially-populated destination.

### What moves

Only `ark.mckinsey.com` resources are backend-specific. Secrets, ConfigMaps, Deployments, and every other core Kubernetes object stay in etcd in both modes. Leave them alone.

| Resources | How |
|---|---|
| Tools, Models, Agents, Teams, MCPServers, A2AServers | `ark export` |
| ExecutionEngines, Memories | Outside the default export set. Dump and sanitise them with `kubectl` and `yq` |
| Queries, A2ATasks | Not migrated. Execution history tied to the resource versions and statuses of one backend. |
...
Tools generated by an MCPServer and Agents generated by an A2AServer are excluded from the export. The destination controller recreates them with valid ownership once the parent resource is imported.
...
For the kinds outside the default export set, strip the server-managed fields yourself. A bare `kubectl get -o yaml` carries `uid`, `resourceVersion`, and `status`, which the destination backend rejects or stores as stale:
...
Now check the export before you go anywhere near step 2, which may be irreversible. `ark export` prints failures to stderr but still exits `0`, and it writes no file at all for a namespace that turned out to hold nothing, so a silent failure looks exactly like a clean run. Compare the counts:

```bash
# Everything, including generated resources. Step 4 compares against this.
kubectl get agents,models,teams,tools,mcpservers,a2aservers -A --no-headers | wc -l

# The exportable subset. The selector drops MCPServer-generated Tools and
# A2AServer-generated Agents, which the export leaves out on purpose.
kubectl get agents,models,teams,tools,mcpservers,a2aservers -A --no-headers \
  -l '!mcp/server,!a2a/server' | wc -l

# One 'kind:' line per exported resource, plus one for the List wrapper in extras.yaml.
cat "$outdir"/*.yaml | grep -c '^kind:'
```

The third number should be the second plus one. If it is short, stop and fix the export. Do not compare it against the first: generated Tools and Agents are excluded by design, so on any cluster running an MCPServer the export is legitimately smaller than the full inventory. Record the first number for step 4, which runs after the controller has recreated them.

Keep the directory until the migration is verified; it is your rollback. `ark export` strips `uid`, `resourceVersion`, `generation`, `creationTimestamp`, `managedFields`, `ownerReferences`, `finalizers`, `status`, and the last-applied annotation, so the files carry desired state only and apply cleanly to a different backend. Export files created by older Ark CLI versions may still carry those fields. Regenerate them rather than importing them.
...
for f in "$outdir"/*.yaml; do ark import "$f"; done
```

`ark import` uses `kubectl create` and fails if any resource already exists. To resume a partial import or re-run the migration, use `--upsert`, which switches to `kubectl apply`:

```bash
for f in "$outdir"/*.yaml; do ark import "$f" --upsert; done
```

`--upsert` overwrites the spec of resources that already exist in the destination. Immutable-field changes and admission-webhook rejections can still fail an import; the command reports how many resources were created, configured, and unchanged before the error.

`kubectl apply` does not prune, so `--upsert` only ever creates or updates. A resource deleted after the export was taken, or deleted in the destination while the other backend was serving, comes back. Delete those explicitly once the import is done.

### 4. Verify

```bash
# Matches the first count from step 1: the full inventory, generated resources
...
Then run a query against a migrated agent to confirm the controller resolves its references end to end.
