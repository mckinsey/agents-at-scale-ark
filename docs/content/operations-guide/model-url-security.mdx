...
**Important:** Use CIDR notation (e.g., `10.0.0.0/24` for subnets, `192.168.1.1/32` for single IPs). See [CIDR Notation Examples](#cidr-notation-examples) and [Common Configurations](#common-configurations-1) below for more details.
...
data:
  allowed-private-ips: |
    # Gateway cluster subnet
    10.100.50.0/24
    # Load balancer subnet
    10.100.51.0/24
```

#### Multi-Region Internal Infrastructure
```yaml
data:
  allowed-private-ips: |
    # US East datacenter
...
#### AWS VPC with Private Endpoints
```yaml
data:
  allowed-private-ips: |
    # VPC CIDR for AI gateway
    10.0.0.0/16

    # Specific private subnet for model hosting
    10.0.10.0/24

    # VPC endpoint for Bedrock (example)
    10.0.5.100/32
```
...
data:
  allowed-private-ips: |
    # GCP VPC network range
    10.128.0.0/16

    # Subnet for Vertex AI private endpoint
    10.128.10.0/24
```
...
data:
  allowed-private-ips: |
    # Self-hosted LLM gateway cluster
    192.168.100.0/24

    # GPU server farm for model inference
    192.168.101.0/24
...
data:
  allowed-private-ips: |
    # AWS VPC (cloud gateway)
    10.0.0.0/16

    # On-premises datacenter (VPN connected)
    192.168.0.0/16
