...
- **ark-api**: `SensitiveDataFilter` redacts `access_token`, `refresh_token`, `client_secret`, `code_verifier`, and `authorization` values. It is attached to the root log handler and the uvicorn access/error loggers, so it applies to **all** ark-api logs — not only the MCP auth flow.
