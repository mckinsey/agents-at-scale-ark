...
| **Broker events (Postgres)** | Operation events, when `backends.event: postgres` is enabled | No. Plaintext in the `events` table (same database as messages). | Enable Transparent Data Encryption (TDE) or use an encrypted volume for the PG data directory. |
| **Broker sessions (Postgres)** | The session read model — participants, conversations, per-query status — when `backends.sessions: postgres` is enabled | No. Plaintext in the `sessions` and `session_queries` tables (same database as messages and events). | Enable Transparent Data Encryption (TDE) or use an encrypted volume for the PG data directory. |
...
- **ark-api**: `SensitiveDataFilter` redacts the keys `access_token`, `refresh_token`, `client_secret`, `code_verifier`, `authorization`, `password`, `passwd`, `api_key`, `apikey`, `api-key`, `secret`, `client_key`, `aws_secret_access_key`, `secret_access_key`, `private_key`, `cookie`, and `token` (matched as a suffix, so `set-cookie`, `x-auth-token`, `x-csrf-token`, `proxy-authorization`, and `x-api-key` are covered too), plus the password segment of URL userinfo (`scheme://user:<secret>@host`, e.g. a database DSN), plus standalone tokens matching known formats (JWTs, provider API keys such as `sk-…`/`ghp_…`/`AKIA…`/`AIza…`, and PEM private-key blocks). It is attached to the root log handler and the uvicorn access/error loggers, so it applies to **all** ark-api logs — not only the MCP auth flow. The OTel trace redactor shares the same patterns and test vectors.
...
For the full statement of what payload content reaches logs at default versus verbose levels, see the [Logging Contract](/operations-guide/logging-contract).
...
| **Broker events (Postgres)** | Operation events expire via a TTL snapshot (`expires_at`) the same way as messages. The TTL aligns with the originating query's TTL (`spec.ttl`); the default `EVENT_VISIBILITY_TTL_SECONDS` is 30 days. Events are also hard-deleted when their originating Query CR is deleted (controller finalizer, via `DELETE /events/{queryId}`). | Tune the query TTL (`ArkConfig.spec.queryTTL`) or `EVENT_VISIBILITY_TTL_SECONDS`. Expired rows remain until purged — schedule a job to hard-delete them if you need to reclaim space. |
| **Broker sessions (Postgres)** | Sessions expire via a sliding `expires_at`, pushed out by `SESSIONS_VISIBILITY_TTL_SECONDS` (default 30 days) on every event or message. Expiry hides a session from reads but does not delete the row. Deleting a Query CR does remove its row from every session holding it (controller finalizer, via `DELETE /sessions/queries/{queryId}`), and a session left with no queries is deleted with it. | Tune `SESSIONS_VISIBILITY_TTL_SECONDS`, which must stay at least as long as the message and event TTLs. Schedule a job to delete rows past `expires_at` if you need to reclaim space. |
...
- **Admission webhooks** — In the **default (etcd/CRD) deployment**, a Kubernetes validating webhook can inspect Query CRD `.spec.input` at creation time to block or flag content that matches DLP rules. **This does not apply in aggregated apiserver mode** (the opt-in PostgreSQL backend): the main kube-apiserver does not run its webhook chain on aggregated resources, so third-party webhooks (Kyverno, OPA/Gatekeeper) do not fire on `ark.mckinsey.com` resources via the host. There, use a native [`ValidatingAdmissionPolicy`](https://kubernetes.io/docs/reference/access-authn-authz/validating-admission-policy/) (CEL, host k8s ≥1.30) — which the aggregated apiserver enforces in-process, including on the direct service path — or set `policy.thirdPartyWebhooks.enabled=true` on the `chart-apiserver` chart to run the webhook admission plugins in the aggregated apiserver itself, which makes your existing Kyverno/Gatekeeper webhook configurations apply to Ark resources on both paths. That is off by default because it puts a synchronous webhook call on every write. The egress proxy above remains an option in either mode.
