...
- **ark-api**: `SensitiveDataFilter` redacts the keys `access_token`, `refresh_token`, `client_secret`, `code_verifier`, and `authorization`, plus standalone tokens matching known formats (JWTs, provider API keys such as `sk-…`/`ghp_…`/`AKIA…`/`AIza…`, and PEM private-key blocks). It is attached to the root log handler and the uvicorn access/error loggers, so it applies to **all** ark-api logs — not only the MCP auth flow. The OTel trace redactor shares the same patterns and test vectors.
