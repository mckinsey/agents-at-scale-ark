...
| **Broker sessions (Postgres)** | Sessions expire via a sliding `expires_at`, pushed out by `SESSIONS_VISIBILITY_TTL_SECONDS` (default 30 days) on every event or message. Expiry hides a session from reads but does not delete the row, and nothing removes a session when its Query CR is deleted. | Tune `SESSIONS_VISIBILITY_TTL_SECONDS`, which must stay at least as long as the message and event TTLs. Schedule a job to delete rows past `expires_at` if you need to reclaim space. |
