...
When several people share an Ark deployment, you often want calls to an external service to run **as the individual user** — their permissions, their rate limits, their audit trail — not as one shared service account. This tutorial shows how, using the GitHub MCP server as the example: the server is registered once, and each query carries the user's own token via an **override**.
