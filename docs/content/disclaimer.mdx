...
Ark does not yet provide a stable API, and breaking changes may occur between releases. Check the [release notes](https://github.com/mckinsey/agents-at-scale-ark/releases) for migration steps. Since Ark functions primarily as an operational layer, the recommended approach is to define your resources declaratively in YAML files or store configuration data in your application layer database, decoupled from Ark itself. This enables you to recreate resources in Ark as needed, allowing the system to reconcile them without data migration concerns.
