...
### 3. Silent fallback on unrecognized member name (issue #1079)
...
### 4. Fragile name matching (issue #1079)
...
### 5. No-outgoing-edges fallback behavior undefined (issue #1083)
...
- (3, 5) When the selector agent fails to select the next conversant, or if no next-conversant can be selected due to the graph topology, the conversation should terminate in a clear error state instead of silently transitioning to the first conversant.
...
- (8) The selector should always have access to the `terminate` tool. If we decide to implement the `select-next-conversant` tool and leverage the Responses API to force a tool call, we can pass both tools as the only available tools.
