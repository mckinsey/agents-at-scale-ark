...
|-------|------|----------|-------------|
| `event_id` | UUID | Yes | Unique identifier for this event (generated by watcher) |
| `correlation_id` | String(255) | Yes | Groups related events (e.g., session ID, query ID) |
| `timestamp` | DateTime(TZ) | Yes | When the event occurred (from source) |
| `severity` | Enum | Yes | DEBUG, INFO, WARNING, ERROR, CRITICAL |
| `type` | String(100) | Yes | Event category (e.g., "query", "workflow", "pod", "k8s_event") |
| `subtype` | String(100) | Yes | Event subcategory (e.g., "execution_start", "execution_complete", "failed", "log") |
| `source_type` | String(100) | Yes | Kind of source (e.g., "ark_controller", "watcher", "service", "user") |
| `source` | String(255) | Yes | Specific source identifier (e.g., "query-controller", "argo-watcher") |
| `version` | String(10) | Yes | Schema version (e.g., "v1") |
| `payload` | JSONB | Yes | Flexible data specific to event type |
...
4. **Kafka Publishing**: Send normalized events to "events" topic with `correlation_id` as partition key
...
5. **Kafka Publishing**: Send normalized events to "events" topic with `correlation_id` as partition key
...
2. **Validate**: Check event schema compliance and version. Discard malformed events or events with unsupported versions (log warning and continue processing batch)
3. **Batch Insert**: Write all valid events to PostgreSQL in single transaction using `ON CONFLICT (event_id) DO NOTHING` to handle duplicate deliveries
4. **Commit Offsets**: Only commit Kafka offsets after successful database write
5. **Broadcast**: Publish events to in-memory pub/sub for GraphQL subscribers
...
1. **Direct Kafka Publishing from Ark Controllers**: Instead of emitting K8s events, controllers could publish directly to Kafka to reduce etcd pressure
