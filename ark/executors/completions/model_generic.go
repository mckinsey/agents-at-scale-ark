...
type ChatCompletionProvider interface {
	ChatCompletion(ctx context.Context, messages []Message, n int64, toolChoice ToolChoice, tools ...[]openai.ChatCompletionToolParam) (*openai.ChatCompletion, error)
	ChatCompletionStream(ctx context.Context, messages []Message, n int64, streamFunc func(*openai.ChatCompletionChunk) error, toolChoice ToolChoice, tools ...[]openai.ChatCompletionToolParam) (*openai.ChatCompletion, error)
...
func (m *Model) ChatCompletion(ctx context.Context, messages []Message, eventStream EventStreamInterface, n int64, toolChoice ToolChoice, tools ...[]openai.ChatCompletionToolParam) (*openai.ChatCompletion, error) {
	if m.Provider == nil {
		return nil, nil
	}
	...
	if eventStream != nil {
		...
		response, err = m.Provider.ChatCompletion(ctx, messages, n, toolChoice, tools...)
	}
...
func (m *Model) HealthCheck(ctx context.Context) error {
	...
	default:
		...
		_, err := m.ChatCompletion(ctx, testMessages, nil, 1, ToolChoiceUnset)
		return err
	}
}
