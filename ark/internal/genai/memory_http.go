...
func NewHTTPMemory(ctx context.Context, k8sClient client.Client, memoryName, namespace string, config Config, memoryRecorder eventing.MemoryRecorder) (MemoryInterface, error) {
	...
	// ConversationId from config - if empty, cluster-memory will generate one
...
func (m *HTTPMemory) AddMessages(ctx context.Context, queryID string, messages []Message) error {
	...
	// Decode response to get the conversation_id
	var response struct {
	...
	// Update the conversation ID if it was generated by cluster-memory
	if response.ConversationID != "" && m.conversationId != response.ConversationID {
