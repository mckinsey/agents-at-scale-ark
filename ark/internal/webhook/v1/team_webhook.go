...
func SetupTeamWebhookWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewWebhookManagedBy(mgr, &arkv1alpha1.Team{}).
		WithDefaulter(&validation.WebhookDefaulter[*arkv1alpha1.Team]{Lookup: lookup}).
		WithValidator(&validation.WebhookValidator[*arkv1alpha1.Team]{V: v}).
		Complete()
}
