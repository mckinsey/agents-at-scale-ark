...
func (r *QueryReconciler) executeQueryAsync(opCtx context.Context, obj arkv1alpha1.Query, namespacedName types.NamespacedName) {
	...
	// ConversationId is passed from spec or will be generated by cluster-memory
	...
	// Store the conversation ID from memory in the query status
	if httpMemory, ok := memory.(*genai.HTTPMemory); ok {
		obj.Status.ConversationId = httpMemory.GetConversationID()
	}
