...
Honouring a declared `spec.serviceAccount` requires the executor's service account to hold `impersonate` on `serviceaccounts`. Without it the query fails rather than falling back to the pod identity — a declared restriction that cannot be applied is an error, not a default.

Scope that grant to the accounts queries are allowed to name, via `resourceNames` on the Role. An unscoped grant lets anything holding the executor's token assume any account in the namespace, including a privileged one, which is a wider capability than the executor needs.
