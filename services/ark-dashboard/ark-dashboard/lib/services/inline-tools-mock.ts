...
export type InlineToolUpdateInput = Omit<InlineToolCreateInput, 'name'>;

export function isInlineTool(tool: Tool): tool is InlineTool {
  return tool.type === 'inline' && Boolean(tool.inline);
}
...
export const inlineToolsMockService = {
  ...
  async update(
    name: string,
    input: InlineToolUpdateInput,
  ): Promise<InlineTool> {
    const tools = read();
    const index = tools.findIndex(tool => tool.name === name);
    if (index === -1) {
      throw new Error(`No inline tool named "${name}"`);
    }
    if (!input.inline.source.trim()) {
      throw new Error('Inline source is required');
    }
    const updated: InlineTool = {
      ...tools[index],
      description: input.description,
      inputSchema: input.inputSchema,
      annotations: input.annotations,
