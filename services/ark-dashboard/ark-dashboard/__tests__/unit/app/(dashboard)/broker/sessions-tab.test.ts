...
describe('Sessions Tab Functionality', () => {
  describe('getSessionDisplayNameFromEntries', () => {
    ...
    it('should return sessionId when first entry has no displayable text', () => {
      ...
      expect(getSessionDisplayNameFromEntries(entries, 'sid-456')).toBe('sid-456');
    });
    ...
    it('should return input from first entry when present', () => {
      ...
      expect(getSessionDisplayNameFromEntries(entries, 'sid-789')).toBe('What is 2+2?');
    });

    it('should return message when present in first entry', () => {
      const entries: StreamEntry[] = [
        {
          id: '1',
          timestamp: '2024-01-15T10:00:00.000Z',
          data: { message: 'Query execution completed' },
        },
      ];
    ...
    it('should prefer input over message when both present', () => {
      const entries: StreamEntry[] = [
        {
          id: '1',
          timestamp: '2024-01-15T10:00:00.000Z',
          data: { input: 'First', message: 'Second' },
        },
      ];
      expect(getSessionDisplayNameFromEntries(entries, 'sid')).toBe('First');
    });

    it('should read from nested data when top-level has no text', () => {
      const entries: StreamEntry[] = [
        {
          id: '1',
          timestamp: '2024-01-15T10:00:00.000Z',
      ...
      expect(getSessionDisplayNameFromEntries(entries, 'sid')).toBe('Nested content');
    });
