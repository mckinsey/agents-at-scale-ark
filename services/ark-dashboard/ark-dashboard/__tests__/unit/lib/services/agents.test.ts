...
describe('agentsService', () => {
  ...
  describe('listWithTools', () => {
    it('should request the with-tools view in a single call', async () => {
      ...
      const result = await agentsService.listWithTools('default');

      expect(apiClient.get).toHaveBeenCalledTimes(1);
      expect(apiClient.get).toHaveBeenCalledWith(`/api/v1/agents`, {
        params: { limit: 100, namespace: 'default', view: 'with-tools' },
      });
  ...
  describe('list', () => {
    it('should fetch all agents without hydrating each item', async () => {
      ...
      expect(apiClient.get).toHaveBeenCalledTimes(1);
      expect(apiClient.get).toHaveBeenCalledWith(`/api/v1/agents`, {
        params: { limit: 100, namespace: 'default', view: 'summary' },
      });
  ...
  describe('getByName', () => {
    ...
    it('should throw other errors', async () => {
      ...
      await expect(
        agentsService.getByName('default', 'test-agent'),
  ...
  describe('getById', () => {
    it('should convert numeric ID to string and call getByName', async () => {
      ...
      expect(apiClient.get).toHaveBeenCalledWith(`/api/v1/agents/123`, {
        params: { namespace: 'default' },
      });
    ...
    it('should handle string IDs', async () => {
      ...
      expect(apiClient.get).toHaveBeenCalledWith(`/api/v1/agents/string-id`, {
        params: { namespace: 'default' },
      });
    });
  });
  ...
  describe('delete', () => {
    ...
    it('should throw other errors', async () => {
      ...
      await expect(
        agentsService.delete('default', 'test-agent'),
      ).rejects.toThrow('Server error');
    });
  });
  ...
  describe('deleteById', () => {
    it('should convert ID to string and call delete', async () => {
      ...
      expect(apiClient.delete).toHaveBeenCalledWith(`/api/v1/agents/123`, {
        params: { namespace: 'default' },
      });
      expect(result).toBe(true);
    });
