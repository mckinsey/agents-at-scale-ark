...
import EditWorkflowTemplatePage from './page';
...
describe('EditWorkflowTemplatePage guard', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders the studio when canUpdate is true', async () => {
    setup({ canUpdate: true });

    render(<EditWorkflowTemplatePage />);
  ...
  it('renders a not-authorized message with a back link and no studio when canUpdate is false', async () => {
    ...
    expect(
      screen.getByRole('link', { name: /back to workflow template/i }),
    ).toBeInTheDocument();
  ...
  it('re-runs the access check when the namespace changes', async () => {
    ...
    const { rerender } = render(<EditWorkflowTemplatePage />);
    ...
    rerender(<EditWorkflowTemplatePage />);
