...
3. **The `kubernetes-mcp-server` provides generic resource access.** A separate change (PR #2536, which must land first) deploys the [`kubernetes-mcp-server`](https://github.com/containers/kubernetes-mcp-server) and registers it as an Ark `MCPServer`. That registration discovers the server's tools and materialises them as `Tool` CRDs (linked to the server by the `mcp/server` label); an Agent's `spec.tools` then references those tools **individually by name** (`{type: mcp, name: resources_list}`) — Ark agents do not reference an `MCPServer` wholesale. Its generic `resources_list` (and `resources_get`) tools list any resource by `apiVersion`/`kind` — so `Agent`, `Model`, `Team`, and `WorkflowTemplate` are all reachable with no Ark-specific tool code. The exact `Tool`-CRD names the author Agent enumerates are produced by — and must match — that registration (PR #2536). It is deployed read-only, which is all this feature needs: the author Agent only reads the catalogue; every write goes through ark-api.
...
The LLM that drives authoring is `kind: Agent`, whose manifest is bundled **in the dashboard** and installed on demand from there (Decision 2 / 15) — not a kubectl sample. Its `spec.prompt` carries the schema crib, the canonical recipes, and the fail-fast rule. Its `spec.tools` enumerate the individual MCP tools (`resources_list`, `resources_get`) the `kubernetes-mcp-server` registration exposes as `Tool` CRDs (Context 3) — referenced by name, not as a wholesale `MCPServer` reference.
...
**`resourceVersion` on overwrite.** Kubernetes `replace` rejects an update to an existing object when the body carries no current `metadata.resourceVersion` (or a stale one) — and `draftYaml` never has a usable one: a new-template collision-overwrite is freshly-authored YAML, and in edit mode the agent's regenerated full-replacement block strips it. So the overwrite path reconciles the `resourceVersion` itself: the PUT handler does a **read-then-replace** — `get` the live object, copy its `metadata.resourceVersion` onto the submitted body, then `replace` — so the caller's YAML need not carry one. (Equivalently, a server-side-apply `PATCH` with a fixed field manager avoids `resourceVersion` entirely; read-then-replace is chosen to mirror the existing create/delete handlers.) This defines the two-tabs-editing-the-same-template behaviour as last-write-wins, which is acceptable for v1 (no versioning).
...
The author Agent's `spec.tools` enumerate these tools by their discovered `Tool`-CRD names (`resources_list`, `resources_get`), each as a `{type: mcp, name: …}` entry (Context 3) — not as a single reference to the `MCPServer`. Those names come from the registration in PR #2536, so the bundled manifest's tool list must match whatever that change names them.
...
The separate change that adds `kubernetes-mcp-server` (PR #2536) wires it into `devspace dev` only and owns the Ark `MCPServer` registration. **That change must land first** — until it does, the author Agent has no MCP tools to ground through and this feature is inert. This feature depends on it at runtime, so it must also be installable in a production Ark deployment. Rather than re-deriving the dev config, add a `services/kubernetes-mcp-server/chart/` umbrella chart that mirrors `services/argo-workflows/chart/`:
...
**Scope boundary:** the Ark `MCPServer` resource that registers this server with the cluster is **owned by the kubernetes-mcp-server change** (PR #2536; Context 3 / Open Question on registration was resolved to that change), not duplicated here. This chart deploys the server image; whatever registration that change ships applies equally in production.
