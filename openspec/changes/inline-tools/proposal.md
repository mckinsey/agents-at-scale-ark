...
- Fix the existing typed Tool PUT bug as part of this work: the handwritten `ToolSpec` omits `mcp`/`builtin` while the update handler replaces `spec` wholesale, so a typed update silently drops those blocks.
...
- `services/ark-api/` — update handwritten Tool DTOs and serialization as well as generated SDK types, including the `mcp`/`builtin` fields the typed update currently drops; cover create/update/read/list and generic resource admission paths.
