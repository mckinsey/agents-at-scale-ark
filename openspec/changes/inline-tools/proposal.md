...
- Publish an internal activator address on Tool status and add `conditions` to Tool, aligning it with every other Ark CRD: a single `Available` condition with a closed reason set and its own `observedGeneration` decides whether the endpoint is usable, while `state` keeps its existing role and gains `Pending`. Small adapters in the Go executor and Python SDK resolve inline Tools into existing MCP client/configuration types. Do not create synthetic MCPServer resources or duplicate Tools.
...
- Keep inline authoring off by default. Require a namespace-scoped dedicated author permission on create and spec updates, with mandatory fail-closed enforcement in both CRD admission and the PostgreSQL-backed API server, and record the admitted author on the Tool with an event on source change. API/dashboard writes require authenticated user impersonation without service-account fallback, including generic resource writes.
...
- Enforce pod hardening with explicit requests below limits, own a runner NetworkPolicy denying egress and restricting backend ingress, and report a Pending conflict when another policy in the namespace widens either. Treat CNI enforcement and node-level PID exhaustion the same way: documented prerequisites and residual risks with end-to-end coverage, not Ark-managed settings or runtime gates.
- Depend on, rather than contain, the typed Tool PUT fix: the handwritten `ToolSpec` omitted `mcp`/`builtin` while the update handler replaces `spec` wholesale, so a typed update silently dropped those blocks. That pre-existing bug is independent of inline tools and ships separately in PR #3507; this change adds `inline` to an already-complete whitelist and must not reintroduce the defect.
...
- `ark/internal/controller/tool_controller.go` and new `ark/internal/inlinetoolactivator/` — reconcile child resources and status, serve metadata-only discovery, activate runners, and handle idle, edit, disable, and delete transitions. Package the activator as a singleton deployment alongside the operator, independently of controller replicas, with its own ServiceAccount bound per namespace as the controller's namespaced rules are.
- `ark/executors/completions/agent_tools.go` and `lib/ark-sdk/gen_sdk/overlay/python/ark_sdk/extensions/query.py` — adapt inline resolution into existing MCP types with UID-qualified connection identities on both paths. Preserve the shared factory path used by direct Tool queries and the SDK's existing resolution identity.
...
- `services/ark-api/` — update handwritten Tool DTOs and serialization as well as generated SDK types to carry `inline`; cover create/update/read/list and generic resource admission paths. The `mcp`/`builtin` fields the typed update used to drop are fixed separately in PR #3507, which is a prerequisite.
...
- `docs/` and `samples/tools/` — runnable in-memory examples, authoring guidance, runtime limits, identity/network prerequisites, PID-risk guidance, and safe disable/rollback steps.
