...
- [x] 5.1 Reconcile the runner NetworkPolicy and detect conflicting policies by reading those selecting the runner's labels; test the optional tenant allow-all policy, the Pending reason naming it, recovery when it is narrowed, and re-evaluation after policy/label changes. Send no probe traffic and add no runtime enforcement gate.
  - [x] 5.1a Isolate the pure policy-conflict decision for security review, with unit tests for selectors, additive ingress/egress, the tenant allow-all policy, and re-evaluation against changed inputs. Document the network boundary and administrator responsibilities.
  - [x] 5.1b Wire the decision into reconciliation and policy/pod-label watches; report `ConflictingNetworkPolicy`, clear the endpoint, and recover when narrowed. Cover status and watch wiring with envtest, and check the boundary on the phase 3 enforcing-CNI profile.
- [x] 5.2 Package a singleton activator alongside the operator using `strategy: Recreate`, with internal-only ingress restrictions and its own ServiceAccount, bound per namespace via RoleBindings honouring `controllerManager.watchNamespaces` as the controller's rules are, requiring a non-empty `controllerManager.watchNamespaces` and failing installation rather than falling back to a cluster-wide binding, limited to the `scale` subresource of owned runner Deployments with no Secret or Tool-spec write access; verify that controller replica count does not create multiple active scaling authorities and that a rollout never runs two activators.
- [x] 5.3 Serve stateless MCP initialization, notifications, ping, and discovery from Tool metadata, advertising an empty-object schema for a Tool with no `inputSchema`; test that connecting/listing attached tools starts no runners, never exposes source, and does not refresh idle timers.
- [x] 5.4 Implement valid-call activation with feature, published-endpoint, current UID/revision and ownership checks and backend connection after readiness; test stale routes, unknown tool names, arbitrary-target rejection, concurrent cold starts, and starting only the selected Tool.
  ...
  - [x] 5.4b Wire valid calls through argument validation, coalesced activation, current-revision readiness, and backend MCP connection. Package the activator only once this call path and its lifecycle are enforced.
  - [x] 5.4c Validate supported languages without looking for interpreters in the activator image; retain interpreter checks in runner startup. Cover all four languages and unsupported-language rejection, including authorization tests in the interpreter-free manager image.
...
- [x] 6.3 Preserve MCP `isError` through shared result handling for existing MCP and inline Tools; test model-visible failure and existing error events without a separate inline invocation implementation.
- [x] 6.4 Verify invocation requires no dedicated author grant or new per-user credential and does not bypass the execution engine's existing resource/allowlist/approval checks; retain ordinary HTTP/MCP authentication behavior.
