...
The controller publishes `status.resolvedAddress` and `status.observedGeneration`, and reports lifecycle through the existing `status.state`/`status.message` pair rather than introducing conditions on Tool. `resolvedAddress` matches the existing `MCPServerStatus` field of the same name; `state` gains `Pending` alongside `Ready`, with the reason in `message`. The address identifies namespace, name, and UID at the shared activator, not the runner Service. A Ready Tool may have zero runner pods. The endpoint is usable only for the current generation; adapters reject or skip unresolved Tools using their existing error-reporting conventions and never fall back to a caller-supplied URL.
