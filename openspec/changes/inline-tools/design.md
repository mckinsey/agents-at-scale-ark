...
- **Activator privilege.** The activator is reachable from executor workloads and can scale runner Deployments in the namespaces it serves, so it is a privileged component on the invocation path. It is bound per namespace rather than cluster-wide, holds only the `scale` subresource on runners it owns, and accepts no caller-supplied backend target — but an administrator who widens its binding or its ingress widens that reach.
...
Update handwritten API request models and dashboard serialization as well as generated SDK types. `update_tool` assigns `spec` wholesale from `ToolSpec`, so any subtype the model omits is dropped from an existing Tool on a typed PUT. That whitelist was already incomplete (`mcp`, `builtin`, `spec.annotations`, and an over-narrow `http`), which is a pre-existing bug independent of inline tools: it is fixed in PR #3507 and that fix is a prerequisite here, so `inline` is added to a complete list rather than a broken one. Support create, detail read, and persisted source/language edits through PUT. Add language to the list projection for the badge without returning every script body. Read the `Available` condition the way `a2a-servers.ts` already reads its own, branching on `reason` so a policy conflict for an administrator reads differently from a runtime the platform team has not installed. Display admission errors; authorization remains server-side.
