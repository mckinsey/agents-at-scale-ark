...
- **Namespace mismatch:** both backends reject a request body whose namespace disagrees with the server-validated request namespace, rather than silently rewriting it. Pinning the namespace before the review is the security requirement either way; rejecting keeps the two paths identical and leaves room to surface the error to the caller.
- **Authorship on metadata-only writes:** an unchanged-spec update has no new authoring event to attribute, so admission copies the stored annotations forward and deletes any authorship key the stored object does not carry. The stored object is therefore the only source of authorship, including for an inline Tool that reached storage without a stamp (webhooks disabled, a direct etcd write, a migration) — a requester cannot name an author by adding the annotation in a no-op spec update.
