...
- Add a single top-level chainsaw test (`tests/streaming-metadata/`) that validates streaming chunk metadata across three query scenarios: model-target, agent-target, and team-target.
...
### New Capabilities
- `streaming-chunk-assertions`: E2e test suite validating that streaming chunks arriving at ark-broker carry correct ark metadata for model, agent, and team query paths.
...
- `tests/streaming-metadata/` — new test directory with chainsaw-test.yaml, mock-llm-values.yaml, 3 hurl files, manifests
- CI/CD — new test added to chainsaw test suite; requires ark-broker with streaming to be pre-deployed
