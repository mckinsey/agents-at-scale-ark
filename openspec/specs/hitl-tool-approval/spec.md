...
Provide a human-in-the-loop (HITL) tool approval gate for Ark agents. Agents can require human approval before executing specific tool calls, pausing the Query in an `input-required` phase until a human approves or rejects the action. Approval requirements are configured per-tool on `AgentTool`, pending approvals are tracked via the existing `A2ATask` CRD, and responses are submitted through the Ark API with real-time event streaming.
...
#### Scenario: input-required accepted as a valid Query phase

- **WHEN** the Query CRD `status.phase` is set to `input-required`
- **THEN** the value SHALL be accepted by the CRD schema validation
